...
type MBEStatus = "SUBMITTED" | "REJECTED" | "FINAL-APPROVED" | "APPROVED" | "PENDING";
...
type SortKey = keyof MBEItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

const STATUS_STYLES: Record<MBEStatus, string> = {
...
export default function MeasurementBookEntryListPage() {
  const router = useRouter();

  const [filters, setFilters] = useState({
    hoRo: "", entityType: "", entity: "", nameOfBuilding: "", status: "",
  });
  ...
  const handleClear = () => {
    setFilters({ hoRo: "", entityType: "", entity: "", nameOfBuilding: "", status: "" });
    setSelectedId(null);
    setCurrentPage(1);
  };
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Measurement Book Entry List</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white">
            <span className="text-primary">{filtered.length}</span> - Measurement Book Entry(s)
          </p>
          <div className="flex flex-wrap items-center gap-2">
            {/* Edit */}
            <button
              onClick={() => { if (selectedId) router.push("/asset-management/modernization/measurement-book-entry/create"); }}
            ...
            {/* View */}
            <button
              onClick={() => { if (selectedId) router.push("/asset-management/modernization/measurement-book-entry/view"); }}
              disabled={selectedId === null}
              className="flex items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
