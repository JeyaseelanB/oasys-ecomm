...
type AllocationStatus = "SUBMITTED" | "FINAL APPROVED";
...
const SAMPLE_DATA: AssetAllocationItem[] = [
  { id: 1, referenceNumber: "AA086", entity: "",           employee: "",          assetName: "Benq-Projector,Benq-Projector", quantity: 2, createdDate: "30-Oct-2023", status: "SUBMITTED"     },
  { id: 2, referenceNumber: "AA085", entity: "HEAD OFFICE", employee: "LAKSHMI N", assetName: "Mahendra Car",                  quantity: 1, createdDate: "20-Mar-2023", status: "FINAL APPROVED" },
  { id: 3, referenceNumber: "AA084", entity: "",           employee: "",          assetName: "Desktop",                       quantity: 1, createdDate: "22-Apr-2021", status: "SUBMITTED"     },
];
...
type SortKey = keyof AssetAllocationItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function AssetAllocationListPage() {
  const router = useRouter();

  const [filters, setFilters] = useState({
    referenceNumber: "", entity: "", employee: "", assetName: "", quantity: "", createdDate: "", status: "",
  });
  ...
  const handleClear = () => {
    setFilters({ referenceNumber: "", entity: "", employee: "", assetName: "", quantity: "", createdDate: "", status: "" });
    setSelectedId(null);
    setCurrentPage(1);
  };
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Asset Allocation List</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white">
            <span className="text-primary">{filtered.length}</span> - Asset Allocation(s)
          </p>
          <div className="flex flex-wrap items-center gap-2">
            {/* Add */}
            <button
              onClick={() => router.push("/asset-management/asset-allocation/create")}
            ...
            {/* Edit */}
            <button
              onClick={() => { if (selectedId) router.push("/asset-management/asset-allocation/create"); }}
              disabled={selectedId === null}
            ...
            {/* View */}
            <button
              onClick={() => { if (selectedId) router.push("/asset-management/asset-allocation/view"); }}
              disabled={selectedId === null}
              className="flex items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("entity")}>
                  Entity <SortIcon col="entity" />
                </th>
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("employee")}>
                  Employee <SortIcon col="employee" />
                </th>
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("assetName")}>
                  Asset Name <SortIcon col="assetName" />
                </th>
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("quantity")}>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white"
                    value={filters.entity} onChange={(e) => { setFilters((f) => ({ ...f, entity: e.target.value })); setCurrentPage(1); }} />
                </td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white"
                    value={filters.employee} onChange={(e) => { setFilters((f) => ({ ...f, employee: e.target.value })); setCurrentPage(1); }} />
                </td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white"
                    value={filters.assetName} onChange={(e) => { setFilters((f) => ({ ...f, assetName: e.target.value })); setCurrentPage(1); }} />
                </td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white"
                    value={filters.quantity} onChange={(e) => { setFilters((f) => ({ ...f, quantity: e.target.value })); setCurrentPage(1); }} />
                </td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <select className="w-full rounded border border-stroke bg-transparent px-1 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white"
                ...
                paginated.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`border-r border-stroke px-3 py-3 dark:border-dark-3 ${isSelected ? "text-white" : "text-dark dark:text-white"}`}>{row.referenceNumber}</td>
                      <td className={`border-r border-stroke px-3 py-3 dark:border-dark-3 ${isSelected ? "text-white" : "text-[#17a2b8]"}`}>{row.entity}</td>
                      <td className={`border-r border-stroke px-3 py-3 dark:border-dark-3 ${isSelected ? "text-white" : "text-[#17a2b8]"}`}>{row.employee}</td>
                      <td className={`border-r border-stroke px-3 py-3 dark:border-dark-3 ${isSelected ? "text-white" : "text-[#17a2b8]"}`}>{row.assetName}</td>
                      <td className={`border-r border-stroke px-3 py-3 text-center dark:border-dark-3 ${isSelected ? "text-white" : "text-[#17a2b8]"}`}>{row.quantity}</td>
                      <td className={`border-r border-stroke px-3 py-3 dark:border-dark-3 ${isSelected ? "text-white" : "text-dark dark:text-white"}`}>{row.createdDate}</td>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setCurrentPage(1)} disabled={currentPage === 1}
              className="flex size-8 items-center justify-center rounded border border-stroke text-sm hover:bg-gray-100 disabled:opacity-40 dark:border-dark-3 dark:hover:bg-dark-2">&#171;</button>
            ...
            <button onClick={() => setCurrentPage(totalPages)} disabled={currentPage === totalPages}
              className="flex size-8 items-center justify-center rounded border border-stroke text-sm hover:bg-gray-100 disabled:opacity-40 dark:border-dark-3 dark:hover:bg-dark-2">&#187;</button>
            <select className="ml-2 rounded border border-stroke bg-transparent px-2 py-1 text-sm outline-none dark:border-dark-3 dark:text-white"
