...
const SECTIONS   = ["Admin","Accounts","Contract section","Export","Internal Audit Wing","Product Development","PRODUCTION WING","Recruitment","Sales","Silk and Handloom"];
...
const TOOLBAR_GROUPS = [
  ["Sans Serif▾","Normal▾"],
  ["B","I","U","S","A▾","A̲▾","x₂","x²","H₁","H₂","❝","</>"],
  ["≡","⋮","⇤","⇥","⇶","≡"],
  ["🔗","🖼","⊞","Tx"],
];

export default function CreateEADPage() {
  const router = useRouter();

  const [type,       setType]       = useState("");
  const [payAspect,  setPayAspect]  = useState("");
  ...
  const [rows, setRows] = useState<EmpRow[]>([
    { id:1, hoRo:"HEAD OFFICE", dept:"ADMIN",     section:"Admin",       employee:"SANKARANARAYANAN",  amount:"", description:"" },
    { id:2, hoRo:"HEAD OFFICE", dept:"ADMIN",     section:"Admin",       employee:"ARULRAJAN",         amount:"", description:"" },
    { id:3, hoRo:"HEAD OFFICE", dept:"MARKETING", section:"Export",      employee:"MURUGESAN",         amount:"", description:"" },
    { id:4, hoRo:"HEAD OFFICE", dept:"FINANCE",   section:"Accounts",    employee:"KRISHNASWAMY",      amount:"", description:"" },
    { id:5, hoRo:"HEAD OFFICE", dept:"HR",        section:"Recruitment", employee:"VIJAYALAKSHMI",     amount:"", description:"" },
  ]);

  const handleGenerate = () => {
    if (!type || !payAspect || !hoRo || !entType || !entity || !year || !month) {
      setError("Please fill all mandatory fields (HO/RO, Entity Type, Entity, Year, Month) before generating.");
      return;
    }
    setError("");
    setGenerated(true);
  ...
  const handleClear = () => {
    setType(""); setPayAspect(""); setHoRo(""); setEntType("");
    setEntity(""); setDept(""); setSection(""); setYear(""); setMonth("");
    setGenerated(false); setError("");
    setRows(rows.map(r => ({ ...r, amount: "", description: "" })));
  };
  ...
  return (
    <div className="mx-auto">
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Create Additional Earnings / Deduction</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Create Additional Earnings / Deduction</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="p-5">
          ...
          <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <div className="flex">
                ...
                <div className="flex h-[42px] w-full cursor-pointer items-center rounded-r border border-stroke bg-white px-3 text-sm text-gray-500 dark:border-dark-3 dark:bg-gray-dark dark:text-gray-400">
          ...
          {/* Row 3 — Year, Month, Buttons inline */}
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="flex gap-2 pb-0.5">
              <button onClick={handleClear} className="flex items-center gap-1.5 rounded bg-[#6c757d] px-4 py-2.5 text-sm font-medium text-white hover:opacity-90">
              ...
              <button onClick={handleGenerate} className="flex items-center gap-1.5 rounded bg-[#2d8f7b] px-4 py-2.5 text-sm font-medium text-white hover:opacity-90">
                <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="mt-5 overflow-x-auto rounded border border-stroke dark:border-dark-3">
            <table className="w-full min-w-[700px] text-sm">
              ...
              <tbody>
                {!generated ? (
                  <tr><td colSpan={7} className="px-4 py-4 text-sm text-gray-500 dark:text-gray-400">No records found.</td></tr>
                ) : rows.map((r, i) => (
          ...
          <div className="w-full max-w-2xl rounded-[10px] border border-stroke bg-white shadow-xl dark:border-dark-3 dark:bg-gray-dark">
            ...
            <div className="p-5">
              ...
              <div className="mb-1 flex flex-wrap items-center gap-0.5 rounded-t border border-b-0 border-stroke bg-gray-50 p-1.5 dark:border-dark-3 dark:bg-gray-700">
                {[
                  ...
                  <span key={gi} className="flex items-center">
                    ...
                    {grp.map(t => (
                      <button key={t} className="flex h-7 min-w-[28px] items-center justify-center rounded px-1.5 text-xs font-medium text-gray-600 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-600">
                        {t}
                      </button>
                    ))}
                  </span>
              ...
              <div className="mt-4 flex items-center gap-3">
                <button className="flex size-8 items-center justify-center rounded-full border border-stroke bg-gray-100 text-gray-500 hover:bg-gray-200 dark:border-dark-3 dark:bg-gray-700">‹</button>
                <div className="flex-1 rounded border border-orange-300 p-4">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">Designation : SUPERINTENDENT</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Date : 12-Mar-2026</p>
                </div>
                <button className="flex size-8 items-center justify-center rounded-full border border-stroke bg-gray-100 text-gray-500 hover:bg-gray-200 dark:border-dark-3 dark:bg-gray-700">›</button>
              </div>
