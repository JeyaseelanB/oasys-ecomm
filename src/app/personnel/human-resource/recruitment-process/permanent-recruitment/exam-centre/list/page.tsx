...
interface DataItem { id: number; region: string; examCenter: string; primaryContactNumber: string; dateOfExamination: string; status: string; }
const SAMPLE_DATA: DataItem[] = [];
type SortKey = keyof DataItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function ExamCentreListPage() {
  const [filters, setFilters] = useState({ region: "", examCenter: "", primaryContactNumber: "", dateOfExamination: "", status: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) => (filters.region === "" || row.region === filters.region) && row.examCenter.toLowerCase().includes(filters.examCenter.toLowerCase()) && row.primaryContactNumber.toLowerCase().includes(filters.primaryContactNumber.toLowerCase()) && row.dateOfExamination.toLowerCase().includes(filters.dateOfExamination.toLowerCase()) && (filters.status === "" || row.status === filters.status));
  ...
  const handleClear = () => { setFilters({ region: "", examCenter: "", primaryContactNumber: "", dateOfExamination: "", status: "" }); setSelectedId(null); setCurrentPage(1); };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Exam Centre List</h2>
        <nav><ol className="flex items-center gap-1.5 text-sm"><li><Link href="/" className="font-medium text-dark hover:text-primary dark:text-gray-400">Home</Link></li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Personnel</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Human Resource</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Recruitment Process</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Permanent Recruitment</li><li className="text-gray-400">/</li><li className="font-medium text-primary">Exam Centre List</li></ol></nav>
      </div>
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white"><span className="text-primary">{filtered.length}</span> - Exam Centre(s)</p>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button disabled={selectedId === null} className="flex items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>Edit</button>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("examCenter")}>Exam center <SortIcon col="examCenter" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("primaryContactNumber")}>Primary Contact Number <SortIcon col="primaryContactNumber" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("dateOfExamination")}>Date Of Examination <SortIcon col="dateOfExamination" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><select className="w-full rounded border border-stroke bg-transparent px-1 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" value={filters.region} onChange={(e) => { setFilters((f) => ({ ...f, region: e.target.value })); setCurrentPage(1); }}><option value="">Select</option></select></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.examCenter} onChange={(e) => { setFilters((f) => ({ ...f, examCenter: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.primaryContactNumber} onChange={(e) => { setFilters((f) => ({ ...f, primaryContactNumber: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.dateOfExamination} onChange={(e) => { setFilters((f) => ({ ...f, dateOfExamination: e.target.value })); setCurrentPage(1); }} /></td>
