...
interface DataItem { id: number; advertisementNumber: string; nameOfPost: string; recruitmentYear: string; createdDate: string; department: string; status: string; }
const SAMPLE_DATA: DataItem[] = [
  { id: 1, advertisementNumber: "1/2024", nameOfPost: "Weaver Gr-II", recruitmentYear: "2024-2025", createdDate: "14/06/2024", department: "Production", status: "Pending" },
  { id: 2, advertisementNumber: "2/2024", nameOfPost: "Weaver Gr-II", recruitmentYear: "2024-2025", createdDate: "20/06/2024", department: "Production", status: "Pending" }
];
type SortKey = keyof DataItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function JobAdvertisementListPage() {
  const [filters, setFilters] = useState({ advertisementNumber: "", nameOfPost: "", recruitmentYear: "", createdDate: "", department: "", status: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) => row.advertisementNumber.toLowerCase().includes(filters.advertisementNumber.toLowerCase()) && row.nameOfPost.toLowerCase().includes(filters.nameOfPost.toLowerCase()) && row.recruitmentYear.toLowerCase().includes(filters.recruitmentYear.toLowerCase()) && row.createdDate.toLowerCase().includes(filters.createdDate.toLowerCase()) && (filters.department === "" || row.department === filters.department) && (filters.status === "" || row.status === filters.status));
  ...
  const handleClear = () => { setFilters({ advertisementNumber: "", nameOfPost: "", recruitmentYear: "", createdDate: "", department: "", status: "" }); setSelectedId(null); setCurrentPage(1); };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Job Advertisement List</h2>
        <nav><ol className="flex items-center gap-1.5 text-sm"><li><Link href="/" className="font-medium text-dark hover:text-primary dark:text-gray-400">Home</Link></li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Personnel</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Human Resource</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Recruitment Process</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Permanent Recruitment</li><li className="text-gray-400">/</li><li className="font-medium text-primary">Job Advertisement List</li></ol></nav>
      </div>
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white"><span className="text-primary">{filtered.length}</span> - Job Advertisement(s)</p>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("nameOfPost")}>Name of Post <SortIcon col="nameOfPost" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.nameOfPost} onChange={(e) => { setFilters((f) => ({ ...f, nameOfPost: e.target.value })); setCurrentPage(1); }} /></td>
