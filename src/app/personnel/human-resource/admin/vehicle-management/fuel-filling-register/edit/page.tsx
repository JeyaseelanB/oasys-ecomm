...
export default function EditFuelFillingRegisterPage() {
  const router = useRouter();

  // Pre-filled with existing record
  const [vehicle] = useState("TN01 AL 47 57");
  const [lastFilledQty] = useState("30.0");
  const [lastFilledKm] = useState("118000.0");
  const [lastFilledDate] = useState("14-Jul-2020");
  const [lastClosingTripKm, setLastClosingTripKm] = useState("");
  const [fillingDate, setFillingDate] = useState("18-Nov-2020");
  const [currentMeterReading, setCurrentMeterReading] = useState("118000.0");
  const [destination, setDestination] = useState("petrol bank");
  const [fuelType, setFuelType] = useState("DIESEL");
  const [couponBookNumber, setCouponBookNumber] = useState("330");
  const [couponStartEnd, setCouponStartEnd] = useState("151 - 200");
  const [couponNumber, setCouponNumber] = useState("187");
  const [fuelToFill, setFuelToFill] = useState("30.0");
  const [pricePerLitre, setPricePerLitre] = useState("");
  const [fuelValueAmount, setFuelValueAmount] = useState("0");
  const [reason, setReason] = useState("");
  const [uploadDocument, setUploadDocument] = useState("IMG_20201118_171742182_20201118_171750492");
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Fuel Filling Register</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Edit Fuel Filling Register</li>
          </ol>
        </nav>
      </div>
        ...
        <div className="p-5">
          ...
          <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <div className="flex">
                ...
                <select value={vehicle} disabled className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-white">
                  <option value="TN01 AL 47 57">TN01 AL 47 57</option>
                </select>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex">
                ...
                <input type="text" value={lastFilledKm} readOnly className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-white" />
              </div>
            </div>
            <div>
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center gap-2">
              <input type="text" value={uploadDocument} readOnly className="w-64 rounded border border-stroke bg-transparent px-3 py-2 text-sm outline-none dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
