...
const TimeSpinner = ({ value, onChange, label, required = false }: {
  ...
  return (
    <div>
      <label className={labelCls}>{label}{required && <span className="ml-0.5 text-red-500">*</span>}</label>
      <div className="flex items-stretch rounded border border-stroke overflow-hidden dark:border-dark-3">
        <span className="flex min-w-[34px] items-center justify-center bg-gray-100 border-r border-stroke text-gray-500 dark:bg-gray-700 dark:border-dark-3"><ClockIcon /></span>
        <input type="text" value={value} onChange={e => onChange(e.target.value)} placeholder="HH:MM" className={inputCls} />
...
export default function EditMeetingRequestPage() {
  const router = useRouter();

  const [form, setForm] = useState({
    type: "Internal", venue: "Madurai", agenda: "jhjhj", meetingDate: "2025-03-26",
    startTime: "03:00", endTime: "10:00", totalAttendees: "1",
    additionalRequirements: "bbgggj", agendaPoints: "ccgcgcggccg",
    forwardTo: "", forwardFor: "Final Approval",
  });

  const [internalRows] = useState([
    { id: 1, name: "KANMANI S", designation: "MANAGER GRADE I", department: "MARKETING", section: "Marketing" },
  ]);

  const [externalRows, setExternalRows] = useState<ExternalRow[]>([
    { id: 1, name: "hhklkhl", org: "hhlhlhllh", mobile: "7899999999", email: "prakash.baalu@g.com", saved: false },
  ]);
  ...
  const saveNote = () => {
    ...
    setNoteContent(""); setShowNoteModal(false);
  };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Meeting Request</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Edit Meeting Request</li>
          </ol>
        </nav>
      </div>
        ...
        <div className="p-5">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <IBox label="Agenda of the Meeting" required icon={<ListIcon />}>
              <input type="text" value={form.agenda} onChange={set("agenda")} className={inputCls} />
            </IBox>
          ...
          <div className="mt-4 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-3">
            ...
            <IBox label="Total Attendees" icon={<HashIcon />}>
              <input type="number" value={form.totalAttendees} readOnly
                className={inputCls + " bg-gray-50 dark:bg-gray-700 cursor-not-allowed"} />
            </IBox>
          </div>
          ...
          <div className="mt-1 flex justify-end text-xs text-gray-500">(1 of 1) &nbsp;
            ...
            <span className="rounded border border-[#2d8f7b] bg-[#2d8f7b] px-2 py-0.5 text-xs text-white">1</span>&nbsp;
            <button className="rounded border border-stroke px-1.5 py-0.5 text-xs disabled:opacity-40">»</button>&nbsp;
                    ...
                    <td className="px-2 py-1.5 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => saveExtRow(row.id)}
                          className="flex h-6 w-6 items-center justify-center rounded bg-[#28a745] text-white hover:opacity-90">
          ...
          <div className="mt-4 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
            <IBox label="Forward to" required icon={<ArrowIcon />}>
              <input type="text" value={form.forwardTo} onChange={set("forwardTo")} className={inputCls} />
            </IBox>
            <IBox label="Forward for" required icon={<ArrowIcon />}>
              <select value={form.forwardFor}
                onChange={e => setForm(f => ({ ...f, forwardFor: e.target.value }))} className={selectCls}>
                <option value="">Select</option>
              ...
              <div className="mb-2 flex flex-wrap gap-1 rounded border border-stroke p-1.5 dark:border-dark-3">
                {[["B","bold"],["I","italic"],["U","underline"],["S","strikeThrough"]].map(([lbl, cmd]) => (
                  <button key={cmd} onMouseDown={e => { e.preventDefault(); execCmd(cmd); }}
                    className="rounded px-2 py-0.5 text-xs font-semibold hover:bg-gray-100 dark:hover:bg-gray-700">{lbl}</button>
                ))}
              </div>
