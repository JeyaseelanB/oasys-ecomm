...
const RTE_TOOLBAR = ["B","I","U","S","x₂","x²","T+","T+","H1","H2","«»","<>","ol","ul","←→","↕","↔","=","↵","🔗","🖼","▶","align"];

export default function CreatePolicyNotePage() {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);
  const editorRef = useRef<HTMLDivElement>(null);
  const [form, setForm] = useState<Record<FormField, string>>({
    govRefNo: "", govRefDate: "", period: "", dueDate: "", department: "", section: "",
    policyDesc: "", uploadFileName: "",
  });
  ...
  const validate = () => {
    ...
    if (!(editorRef.current?.innerText?.trim())) e.policyDesc = "Required";
    if (!form.uploadFileName) e.uploadFileName = "Required";
    return e;
  };
  ...
  const InputWithIcon = ({ icon, field, type="text", placeholder="" }: {
    icon: React.ReactNode; field: FormField; type?: string; placeholder?: string;
  }) => (
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Create Policy Note</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Create Policy Note</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="space-y-4 p-5">
          ...
          <div>
            ...
            <div className={`rounded border ${errors.policyDesc?"border-red-400":"border-stroke dark:border-dark-3"}`}>
              {/* Toolbar */}
              <div className="flex flex-wrap items-center gap-0.5 border-b border-stroke bg-gray-50 px-2 py-1.5 dark:border-dark-3 dark:bg-gray-800">
                {["B","I","U","abc","x₂","x²","T↑","T↑","H1","T↓","T↑","I"].map((t,i)=>(
                  <button key={i} type="button" onMouseDown={e=>{e.preventDefault();document.execCommand(
                    t==="B"?"bold":t==="I"?"italic":t==="U"?"underline":"bold"
                  );}}
                    className="rounded px-1.5 py-0.5 text-xs font-medium text-gray-600 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-700">{t}</button>
                ))}
              ...
              <div
                ...
                onInput={() => { if (errors.policyDesc) setErrors(e=>{const n={...e};delete n.policyDesc;return n;}); }}
                className="min-h-[160px] px-3 py-2 text-sm text-dark focus:outline-none dark:text-white"
              />
            </div>
            {errors.policyDesc && <p className={errCls}>{errors.policyDesc}</p>}
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input type="text" readOnly value={form.uploadFileName} placeholder=""
                className={`w-56 rounded border ${errors.uploadFileName?"border-red-400":"border-stroke dark:border-dark-3"} bg-gray-50 px-3 py-2 text-sm text-dark dark:bg-gray-800 dark:text-white`} />
            ...
            {errors.uploadFileName && <p className={errCls}>{errors.uploadFileName}</p>}
          </div>
