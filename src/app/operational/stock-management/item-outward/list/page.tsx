...
type OutwardStatus = "SUBMITTED" | "INITIATED";

interface ItemOutwardItem {
  id: number;
  entityType: string;
  section: string;
  createdDate: string;
...
const SAMPLE_DATA: ItemOutwardItem[] = [
  { id: 1, entityType: "1228 / TINDIVANAM", section: "", createdDate: "30-Jul-2019", status: "SUBMITTED" },
  { id: 2, entityType: "2028 / V.O.C-TUTICORIN", section: "", createdDate: "29-Jul-2019", status: "SUBMITTED" },
  { id: 3, entityType: "2028 / V.O.C-TUTICORIN", section: "", createdDate: "28-Jul-2019", status: "INITIATED" },
  { id: 4, entityType: "2028 / V.O.C-TUTICORIN", section: "", createdDate: "28-Jul-2019", status: "SUBMITTED" },
  { id: 5, entityType: "2028 / V.O.C-TUTICORIN", section: "", createdDate: "28-Jul-2019", status: "SUBMITTED" },
  { id: 6, entityType: "1916 / PERAMBALUR", section: "", createdDate: "26-Jul-2019", status: "SUBMITTED" },
];

type SortKey = keyof ItemOutwardItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

const STATUS_STYLE: Record<OutwardStatus, string> = {
  SUBMITTED: "bg-[#28a745]",
  INITIATED: "bg-[#17a2b8]",
};

export default function ItemOutwardListPage() {
  const router = useRouter();
  const [filters, setFilters] = useState({ entityType: "", section: "", createdDate: "", status: "" });
  ...
  const handleClear = () => {
    setFilters({ entityType: "", section: "", createdDate: "", status: "" });
    setSelectedId(null);
    setCurrentPage(1);
  };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Stock Item Outward List</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Stock Item Outward List</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white">
            <span className="text-primary">{filtered.length}</span> - Stock Item Outward(s)
          </p>
          <div className="flex flex-wrap items-center gap-2">
            <Link href="/operational/stock-management/item-outward/create" className="flex items-center gap-1.5 rounded bg-[#28a745] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
            ...
            <button onClick={() => { if (selectedId) router.push("/operational/stock-management/item-outward/view"); }} disabled={selectedId === null} className="flex items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("entityType")}>Entity Type <SortIcon col="entityType" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("section")}>Section <SortIcon col="section" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.entityType} onChange={(e) => { setFilters((f) => ({ ...f, entityType: e.target.value })); setCurrentPage(1); }} />
                </td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.section} onChange={(e) => { setFilters((f) => ({ ...f, section: e.target.value })); setCurrentPage(1); }} />
                </td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((row, idx) => (
                  <tr key={row.id} onClick={() => setSelectedId(row.id)} className={`cursor-pointer border-b border-stroke dark:border-dark-3 ${selectedId === row.id ? "bg-[#e8f4f8] dark:bg-[#1e2d42]" : idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"} hover:bg-blue-50 dark:hover:bg-[#1e2d42]`}>
                    ...
                    <td className="border-r border-stroke px-3 py-3 text-dark dark:border-dark-3 dark:text-white">{row.section || ""}</td>
