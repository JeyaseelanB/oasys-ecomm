...
type TenderApplyStatus = "TENDER APPLIED" | "PENDING";

interface ApplyingTenderItem {
  id: number;
  name: string;
  referenceNo: string;
  type: string;
  ...
  status: TenderApplyStatus;
}

const SAMPLE_DATA: ApplyingTenderItem[] = [
  { id: 1, name: "Trial Tender", referenceNo: "12345", type: "Open Tender", category: "Printing and Publishing", supplierCode: "422098", supplierName: "VEERAVANALLUR WEAVERS CO-OP SOCIETY", bidOpeningDate: "28-Nov-2022", bidClosingDate: "30-Nov-2022", bidClosingTime: "12:00:00", status: "TENDER APPLIED" },
  { id: 2, name: "Building", referenceNo: "123", type: "Closed Tender", category: "Paper and Packaging", supplierCode: "321440", supplierName: "KALAIMAGAL WCS. R.H.125", bidOpeningDate: "04-Nov-2022", bidClosingDate: "30-Nov-2022", bidClosingTime: "12:00:00", status: "TENDER APPLIED" },
  { id: 3, name: "Building", referenceNo: "123", type: "Closed Tender", category: "Paper and Packaging", supplierCode: "361607", supplierName: "SAMUNDESWARI W.C.S.T.R.(H) 123", bidOpeningDate: "04-Nov-2022", bidClosingDate: "30-Nov-2022", bidClosingTime: "12:00:00", status: "TENDER APPLIED" },
];

const STATUS_STYLES: Record<TenderApplyStatus, string> = {
  "TENDER APPLIED": "bg-[#28a745] text-white",
  PENDING: "bg-[#FFA70B] text-white",
};

type SortKey = keyof ApplyingTenderItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function ApplyingTenderListPage() {
  const router = useRouter();
  const [filters, setFilters] = useState({ name: "", referenceNo: "", type: "", category: "", supplierCode: "", supplierName: "", bidOpeningDate: "", bidClosingDate: "", bidClosingTime: "", status: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) =>
    ...
    (filters.bidClosingTime === "" || row.bidClosingTime.toLowerCase().includes(filters.bidClosingTime.toLowerCase())) &&
  ...
  const handleClear = () => { setFilters({ name: "", referenceNo: "", type: "", category: "", supplierCode: "", supplierName: "", bidOpeningDate: "", bidClosingDate: "", bidClosingTime: "", status: "" }); setSelectedId(null); setCurrentPage(1); };
  ...
  const basePath = "/operational/tender/apply-tender";

  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Applying Tender List</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Applying Tender List</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white"><span className="text-primary">{filtered.length}</span> - Applying Tender(s)</p>
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => { if (selectedId) router.push(`${basePath}/create`); }} disabled={selectedId === null} className="flex items-center gap-1.5 rounded bg-[#28a745] px-3 py-2 text-xs font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
              <svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22,4 12,14.01 9,11.01"/></svg>
              Apply
            </button>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("name")}>Name <SortIcon col="name" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("referenceNo")}>Reference # <SortIcon col="referenceNo" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("type")}>Type <SortIcon col="type" /></th>
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("supplierCode")}>Supplier Code <SortIcon col="supplierCode" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("supplierName")}>Supplier Name <SortIcon col="supplierName" /></th>
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("bidClosingTime")}>Bid Closing Time <SortIcon col="bidClosingTime" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.name} onChange={(e) => { setFilters((f) => ({ ...f, name: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.referenceNo} onChange={(e) => { setFilters((f) => ({ ...f, referenceNo: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><select className="w-full rounded border border-stroke bg-transparent px-1 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" value={filters.type} onChange={(e) => { setFilters((f) => ({ ...f, type: e.target.value })); setCurrentPage(1); }}><option value="">Sele</option><option value="Open Tender">Open Tender</option><option value="Closed Tender">Closed Tender</option></select></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><select className="w-full rounded border border-stroke bg-transparent px-1 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" value={filters.category} onChange={(e) => { setFilters((f) => ({ ...f, category: e.target.value })); setCurrentPage(1); }}><option value="">Select</option><option value="Printing and Publishing">Printing and Publishing</option><option value="Paper and Packaging">Paper and Packaging</option></select></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.supplierCode} onChange={(e) => { setFilters((f) => ({ ...f, supplierCode: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.supplierName} onChange={(e) => { setFilters((f) => ({ ...f, supplierName: e.target.value })); setCurrentPage(1); }} /></td>
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><div className="flex items-center gap-1"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.bidClosingTime} onChange={(e) => { setFilters((f) => ({ ...f, bidClosingTime: e.target.value })); setCurrentPage(1); }} /><svg className="size-3.5 shrink-0 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10"/><polyline points="12,6 12,12 16,14"/></svg></div></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><select className="w-full rounded border border-stroke bg-transparent px-1 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" value={filters.status} onChange={(e) => { setFilters((f) => ({ ...f, status: e.target.value })); setCurrentPage(1); }}><option value="">Select</option><option value="TENDER APPLIED">TENDER APPLIED</option><option value="PENDING">PENDING</option></select></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"></td>
              </tr>
            </thead>
            <tbody>
              {paginated.length === 0 ? (<tr><td colSpan={12} className="py-8 text-center text-gray-400">No records found</td></tr>) : (
                paginated.map((row, idx) => (
                  <tr key={row.id} className={`border-b border-stroke dark:border-dark-3 ${idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"} hover:bg-blue-50 dark:hover:bg-[#1e2d42]`}>
                    ...
                    <td className="border-r border-stroke px-2 py-3 text-dark dark:border-dark-3 dark:text-white">{row.name}</td>
                    ...
                    <td className="border-r border-stroke px-2 py-3 text-center text-dark dark:border-dark-3 dark:text-white">{row.type}</td>
                    <td className="border-r border-stroke px-2 py-3 text-center text-dark dark:border-dark-3 dark:text-white">{row.category}</td>
                    <td className="border-r border-stroke px-2 py-3 text-center text-dark dark:border-dark-3 dark:text-white">{row.supplierCode}</td>
                    <td className="border-r border-stroke px-2 py-3 text-dark dark:border-dark-3 dark:text-white">{row.supplierName}</td>
                    ...
                    <td className="border-r border-stroke px-2 py-3 text-center text-dark dark:border-dark-3 dark:text-white">{row.bidClosingTime}</td>
