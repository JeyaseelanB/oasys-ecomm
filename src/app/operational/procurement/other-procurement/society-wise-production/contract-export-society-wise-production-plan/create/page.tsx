...
const INITIAL_SOCIETIES: SocietyRow[] = [
  { id: 1,  code: "111147/ARIGNAR ANNA SILK WEAVERS COOP. SOCIETY, K.H.1,",               openingQty: "50.00",  currentQty: "0.00" },
  { id: 2,  code: "111184/KANCHEEPURAMDR. KALAIGNARKARUNANIDHISILWCS.KH.4,",              openingQty: "30.00",  currentQty: "0.00" },
  { id: 3,  code: "111202/KANCHEEPURAM SILK WEAVERS COOP.SOCIETY G.1099",                 openingQty: "20.00",  currentQty: "0.00" },
  { id: 4,  code: "111226/KANCHEEPURAM KAMAKSHIAMMAN SILK WCS. G.1612, NO 5,",            openingQty: "0.00",   currentQty: "0.00" },
  { id: 5,  code: "111263/KANCHEEPURAM MURUGAN SILK WCS. G.1653,",                        openingQty: "0.00",   currentQty: "0.00" },
  { id: 6,  code: "111317/KANCHEEPURAM PALLAVAR SILK WCS. K.P.(SPL) 83,",                 openingQty: "15.00",  currentQty: "0.00" },
  { id: 7,  code: "111408/KANCHEEPURAM THIRUVALLUVAR SILK WCS., G.2054,",                 openingQty: "0.00",   currentQty: "0.00" },
  { id: 8,  code: "111410/SRI VARADARAJASWAMI SILK WCS. G.2105,",                         openingQty: "0.00",   currentQty: "0.00" },
  { id: 9,  code: "112760/KH.277.KPM.PURATCHI THALAIVI DR.J.JAYALALITHA ALL WOMEN WCS",   openingQty: "0.00",   currentQty: "0.00" },
  { id: 10, code: "113181/K.H.306,KANCHEEPURAM PURATCHI PUYAL VAIKO SILK",                openingQty: "10.00",  currentQty: "0.00" },
  { id: 11, code: "272362/ATHIMALAIPATTU ARIGNAR ANNA W.C.S.LTD., VH.9,",                 openingQty: "0.00",   currentQty: "0.00" },
];
...
export default function CreateContractExportSocietyWisePlanPage() {
  ...
  const handleGenerate = () => {
    ...
    setSocieties(rows);
    setGenerated(true);
    const oTotal = rows.reduce((sum, s) => sum + parseFloat(s.openingQty), 0);
    setOpeningTotal(oTotal);
    setCurrentReqQty(0);
  };

  const handleClearForm = () => {
    setCircleCode(""); setProductCategory(""); setProductGroup(""); setProductVariety(""); setUom("");
    setSocieties([]); setGenerated(false); setOpeningTotal(0); setCurrentReqQty(0);
  };
  ...
  const totalOpening = societies.reduce((s, r) => s + (parseFloat(r.openingQty) || 0), 0);
  const totalCurrent = societies.reduce((s, r) => s + (parseFloat(r.currentQty) || 0), 0);
  const totalQty     = totalOpening + totalCurrent;

  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Create Contract / Export Society Wise Production Plan</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="text-gray-500">Create Gov Society Wise Production</li>
        ...
        <div className="p-5">
            ...
            <div>
              ...
              <div className="flex">
                ...
                <select value={planType} onChange={(e) => setPlanType(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  ...
                  <option value="Contract Production Plan">Contract Production Plan</option>
                  <option value="Export Production Plan">Export Production Plan</option>
            ...
            <div>
              ...
              <div className="flex">
                ...
                <select value={planCode} onChange={(e) => setPlanCode(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  <option value="CEP-SEP-25-01 / Contract Silk Plan Sep">CEP-SEP-25-01 / Contract Silk Plan Sep</option>
                  <option value="CEP-AUG-25-11 / Contract Silk Plan Aug">CEP-AUG-25-11 / Contract Silk Plan Aug</option>
                </select>
              </div>
            </div>
            <div>
          ...
          <div className="mt-5">
            ...
            <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-4">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Circle Code/Name</label>
                <div className="flex">
                  <IconBox>
                    <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><line x1="4" y1="9" x2="20" y2="9"/><line x1="4" y1="15" x2="20" y2="15"/><line x1="10" y1="3" x2="8" y2="21"/><line x1="16" y1="3" x2="14" y2="21"/></svg>
                  </IconBox>
                  <select value={circleCode} onChange={(e) => setCircleCode(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                    <option value="">Select</option>
                    <option value="01 / KANCHIPURAM CIRCLE">01 / KANCHIPURAM CIRCLE</option>
                    <option value="02 / SALEM CIRCLE">02 / SALEM CIRCLE</option>
                    <option value="03 / COIMBATORE CIRCLE">03 / COIMBATORE CIRCLE</option>
                  </select>
                </div>
              </div>
              <div>
          ...
          {/* ── Society Wise Production Plan Details (table section) ── */}
          <div className="mt-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              {generated && (
                <div className="flex items-center gap-4 text-sm font-semibold">
                  <span className="text-[#17a2b8]">Opening Stock Quantity : {openingTotal.toFixed(2)}</span>
                  <span className="text-[#28a745]">Current Requirement Quantity : {currentReqQty.toFixed(2)}</span>
                </div>
              )}
              {!generated && (
                <div className="flex items-center gap-4 text-sm font-semibold">
                  <span className="text-[#17a2b8]">Opening Stock Quantity :</span>
                  <span className="text-[#28a745]">Current Requirement Quantity :</span>
                </div>
              )}
            </div>
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {societies.length === 0 ? (
                    ...
                    societies.map((s, idx) => {
                      const rowTotal = (parseFloat(s.openingQty) || 0) + (parseFloat(s.currentQty) || 0);
                      return (
                        <tr key={s.id} className={idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"}>
                          ...
                          <td className="border border-stroke px-2 py-2 text-right align-middle dark:border-dark-3">
                            <div className="rounded border border-stroke bg-gray-50 px-2 py-1 text-right text-sm text-gray-600 dark:border-dark-3 dark:bg-dark-2 dark:text-gray-400">{parseFloat(s.openingQty).toFixed(2)}</div>
                          </td>
                          ...
                          <td className="border border-stroke px-2 py-2 text-right align-middle text-dark dark:border-dark-3 dark:text-white">{rowTotal.toFixed(2)}</td>
                        </tr>
                      );
                    })
                  )}
            ...
            <div className="p-5">
              ...
              <div className="rounded-t border border-stroke dark:border-dark-3">
                <div className="flex flex-wrap items-center gap-0.5 border-b border-stroke bg-[#f9fafb] px-2 py-1.5 dark:border-dark-3 dark:bg-[#1a2232]">
                  ...
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-dark-2"><span className="text-sm font-bold line-through">A</span></button>
