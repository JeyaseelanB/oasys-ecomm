...
interface PlanItem {
  ...
  dpOfficeCodeName: string;
  status: PlanStatus;
}

const SAMPLE_DATA: PlanItem[] = [
  { id: 1, planCodeName: "Prodcution plan",   productVarietyCodeName: "SKBS / SAREES KPM SILK WITH BLOUSE", dpOfficeCodeName: "2107 / D&P OFFICE KANCHIPURAM", status: "FINAL_APPROVED" },
  { id: 2, planCodeName: "dilli",             productVarietyCodeName: "SKBS / SAREES KPM SILK WITH BLOUSE", dpOfficeCodeName: "2107 / D&P OFFICE KANCHIPURAM", status: "SUBMITTED" },
  { id: 3, planCodeName: "dilli",             productVarietyCodeName: "SKBS / SAREES KPM SILK WITH BLOUSE", dpOfficeCodeName: "2107 / D&P OFFICE KANCHIPURAM", status: "FINAL_APPROVED" },
  { id: 4, planCodeName: "SilkPlan_2025",     productVarietyCodeName: "SKBS / SAREES KPM SILK WITH BLOUSE", dpOfficeCodeName: "2107 / D&P OFFICE KANCHIPURAM", status: "FINAL_APPROVED" },
  { id: 5, planCodeName: "KPM_Q2_2025",       productVarietyCodeName: "SKBS / SAREES KPM SILK WITH BLOUSE", dpOfficeCodeName: "2107 / D&P OFFICE KANCHIPURAM", status: "SUBMITTED" },
];
...
type SortKey = keyof PlanItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function AdditionalSocietyWisePlanListPage() {
  ...
  const [filters, setFilters] = useState({ planCodeName: "", productVarietyCodeName: "", dpOfficeCodeName: "", status: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) =>
    ...
    row.productVarietyCodeName.toLowerCase().includes(filters.productVarietyCodeName.toLowerCase()) &&
    row.dpOfficeCodeName.toLowerCase().includes(filters.dpOfficeCodeName.toLowerCase()) &&
  ...
  const handleClear = () => {
    setFilters({ planCodeName: "", productVarietyCodeName: "", dpOfficeCodeName: "", status: "" });
    setSelectedId(null);
    setCurrentPage(1);
  };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Additional Society Wise Production Plan List</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Other Procurement</li>
            <li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Society Wise Production</li>
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Additional Society Wise Production Plan List</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => { if (selectedId) router.push(`${basePath}/edit`); }}
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center align-middle font-semibold hover:bg-[#267a68]" onClick={() => handleSort("productVarietyCodeName")}>Product Variety Code / Name <SortIcon col="productVarietyCodeName" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center align-middle font-semibold hover:bg-[#267a68]" onClick={() => handleSort("dpOfficeCodeName")}>D&amp;P Office Code / Name <SortIcon col="dpOfficeCodeName" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.productVarietyCodeName} onChange={(e) => { setFilters((f) => ({ ...f, productVarietyCodeName: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.dpOfficeCodeName} onChange={(e) => { setFilters((f) => ({ ...f, dpOfficeCodeName: e.target.value })); setCurrentPage(1); }} /></td>
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((row, idx) => (
                  <tr key={row.id} className={`border-b border-stroke dark:border-dark-3 ${idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"} hover:bg-blue-50 dark:hover:bg-[#1e2d42]`}>
                    <td className="border-r border-stroke px-2 py-3 text-center align-middle font-medium text-[#17a2b8] dark:border-dark-3">{(currentPage - 1) * pageSize + idx + 1}</td>
                    ...
                    <td className="border-r border-stroke px-2 py-3 align-middle text-dark dark:border-dark-3 dark:text-white">{row.productVarietyCodeName}</td>
                    <td className="border-r border-stroke px-2 py-3 align-middle text-dark dark:border-dark-3 dark:text-white">{row.dpOfficeCodeName}</td>
