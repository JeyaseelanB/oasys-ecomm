...
// Prefilled data for edit
const PREFILL = {
  planName: "Production plan",
  planFrom: "01-Sep-2025",
  planTo: "09-Sep-2025",
  planFor: "Showroom",
  planDueDate: "16-Sep-2025",
...
const PREFILL_PRODUCTS: ProductItem[] = [
...
export default function EditAdditionalProductionPlanPage() {
  const router = useRouter();
  const basePath = "/operational/production-planning/additional-production-plan";

  // Plan Details (prefilled)
  const [planName, setPlanName] = useState(PREFILL.planName);
  const [planFrom, setPlanFrom] = useState(PREFILL.planFrom);
  const [planTo, setPlanTo] = useState(PREFILL.planTo);
  const [planFor, setPlanFor] = useState(PREFILL.planFor);
  const [planDueDate, setPlanDueDate] = useState(PREFILL.planDueDate);

  // Plan Request Details (prefilled)
  const [requestFrom, setRequestFrom] = useState(PREFILL.requestFrom);
  const [requestDate, setRequestDate] = useState(PREFILL.requestDate);
  ...
  const [products, setProducts] = useState<ProductItem[]>(PREFILL_PRODUCTS);

  // Forward (prefilled)
  const [forwardTo, setForwardTo] = useState(PREFILL.forwardTo);
  const [forwardFor, setForwardFor] = useState(PREFILL.forwardFor);
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Additional Production Plan</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Edit Additional Production Plan</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex items-center justify-between rounded-t-[10px] bg-[#2d8f7b] px-5 py-3">
          <h3 className="text-sm font-semibold text-white">Edit Additional Production Plan</h3>
