...
export default function CreateStockInwardPage() {
  ...
  const [entityCodeName, setEntityCodeName] = useState("");
  const [stockOutwardNumberDate, setStockOutwardNumberDate] = useState("");
  const [bundleNumber, setBundleNumber] = useState("");
  const [qrCode, setQrCode] = useState("");
  const [products, setProducts] = useState<ProductItem[]>([]);
  // Dispatched Bundle
  const [dTotalBundles, setDTotalBundles] = useState("");
  const [dTotalWeight, setDTotalWeight] = useState("");
  const [dBundleNumber, setDBundleNumber] = useState("");
  // Received Bundle
  const [rTotalBundles, setRTotalBundles] = useState("");
  const [rTotalWeight, setRTotalWeight] = useState("");
  const [rBundleNumber, setRBundleNumber] = useState("");
  // Transport
  ...
  const handleAdd = () => {
    if (bundleNumber.trim() || qrCode.trim()) {
      setProducts([{ id: 1, productVarietyCodeName: "BEA4 / BEDSHEET 60X90 ACRYLIC JACQUARD", uom: "NOS", dispatchedQuantity: 0.0, receivedUnit: 140.0, value: 1240.00 }]);
    }
  };

  const totalDispatched = products.reduce((s, p) => s + p.dispatchedQuantity, 0);
  const totalReceived = products.reduce((s, p) => s + p.receivedUnit, 0);
  const totalValue = products.reduce((s, p) => s + p.value, 0);

  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Create Stock Inward</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Create Stock Inward</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="rounded-t-[10px] bg-[#2d8f7b] px-5 py-3">
          <h3 className="text-sm font-semibold text-white">Stock Movement Details</h3>
        </div>

        <div className="p-5">
          <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Type</label>
              <div className="flex">
            ...
            <div>
              ...
              <div className="flex">
                ...
                <select value={entityCodeName} onChange={(e) => setEntityCodeName(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  <option value="2381/PWH CHENNIMALAI">2381/PWH CHENNIMALAI</option>
                  <option value="1881/PWH - SALEM">1881/PWH - SALEM</option>
                  <option value="2181/PWH-KANCHIPURAM">2181/PWH-KANCHIPURAM</option>
                </select>
              </div>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Stock Outward Number / Date <span className="text-red-500">*</span></label>
              <div className="flex">
                <IconBox><span className="text-sm font-bold">#</span></IconBox>
                <select value={stockOutwardNumberDate} onChange={(e) => setStockOutwardNumberDate(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  <option value="2024-358054 / 22-Feb-2024">2024-358054 / 22-Feb-2024</option>
                  <option value="2024-357831 / 14-Feb-2024">2024-357831 / 14-Feb-2024</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 flex items-center justify-end gap-3">
            <button onClick={() => { setType(""); setEntityCodeName(""); setStockOutwardNumberDate(""); setProducts([]); }} className="flex items-center gap-1.5 rounded bg-[#6c757d] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
              <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              Cancel
            </button>
            <button onClick={handleAdd} className="flex items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
          ...
          {/* Bundle Number / QR Code */}
          <div className="mt-5 mb-4 flex flex-wrap items-end gap-3">
            <div>
              ...
              <div className="flex">
                ...
                <select value={bundleNumber} onChange={(e) => setBundleNumber(e.target.value)} className="w-48 rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  <option value="BDL-001">BDL-001</option>
                </select>
              </div>
            </div>
            <div>
          ...
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {products.length === 0 ? (
                    <tr><td colSpan={7} className="border border-stroke py-4 px-3 text-left text-gray-400 dark:border-dark-3">No records found.</td></tr>
                  ) : (
                    products.map((item, idx) => (
                      <tr key={item.id} className={idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"}>
                        ...
                        <td className="border border-stroke px-2 py-2 text-right text-dark dark:border-dark-3 dark:text-white">{item.dispatchedQuantity.toFixed(1)}</td>
                        <td className="border border-stroke px-2 py-2 text-right text-dark dark:border-dark-3 dark:text-white">{item.receivedUnit.toFixed(1)}</td>
                ...
                <tfoot>
                  <tr className="bg-gray-100 dark:bg-[#1a2232]">
                    ...
                    <td className="border border-stroke px-2 py-2 dark:border-dark-3"></td>
                    <td className="border border-stroke px-2 py-2 text-right font-semibold text-dark dark:border-dark-3 dark:text-white">{totalDispatched > 0 ? totalDispatched.toFixed(1) : "0"}</td>
                    <td className="border border-stroke px-2 py-2 text-right font-semibold text-dark dark:border-dark-3 dark:text-white">{totalReceived > 0 ? totalReceived.toFixed(1) : "0"}</td>
          ...
          {/* Bundle Details - Dispatched & Received side by side */}
            ...
            <div>
              ...
              <div className="mb-3 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="flex">
                    <IconBox><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M4 7l8-4 8 4M4 7v10l8 4M4 7l8 4M20 7v10l-8 4M20 7l-8 4M12 11v10"/></svg></IconBox>
                    <input type="text" value={dTotalBundles} onChange={(e) => setDTotalBundles(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex">
                    ...
                    <input type="text" value={dTotalWeight} onChange={(e) => setDTotalWeight(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                  </div>
                </div>
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Bundle Number</label>
                <input type="text" value={dBundleNumber} onChange={(e) => setDBundleNumber(e.target.value)} className="w-full rounded border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
              </div>
            </div>
            <div>
              <div className="mb-3 flex items-center gap-2">
              ...
              <div className="mb-3 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="flex">
                    <IconBox><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M4 7l8-4 8 4M4 7v10l8 4M4 7l8 4M20 7v10l-8 4M20 7l-8 4M12 11v10"/></svg></IconBox>
                    <input type="text" value={rTotalBundles} onChange={(e) => setRTotalBundles(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                  </div>
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Total Bundle Weight</label>
                  <div className="flex">
                    <IconBox><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M12 3v18M5.5 8.5l13-1M5.5 15.5l13 1"/><circle cx="12" cy="12" r="1"/></svg></IconBox>
                    <input type="text" value={rTotalWeight} onChange={(e) => setRTotalWeight(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                  </div>
                </div>
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Bundle Number</label>
                <input type="text" value={rBundleNumber} onChange={(e) => setRBundleNumber(e.target.value)} className="w-full rounded border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-stroke pt-4 dark:border-dark-3">
            <button onClick={() => router.push("/operational/warehouse-management/distribution-warehouse/stock-inward/list")} className="flex items-center gap-1.5 rounded bg-[#dc3545] px-5 py-2.5 text-sm font-medium text-white hover:opacity-90">
