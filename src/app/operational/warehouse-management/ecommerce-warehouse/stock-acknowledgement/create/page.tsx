...
export default function CreateStockAcknowledgementPage() {
  const router = useRouter();
  const [fromEntity, setFromEntity] = useState("");
  const [stockInwardFrom, setStockInwardFrom] = useState("");
  const [stockMovementNumberDate, setStockMovementNumberDate] = useState("/");
  ...
  const totalRate = products.reduce((s, p) => s + p.totalRate, 0);
  const materialValue = totalRate;
  const cgst = 0;
  const sgst = 0;
  const netTotal = materialValue + cgst + sgst;

  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Create Stock Acknowledgement</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Create Stock Acknowledgement</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="p-5">
          <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-dark dark:text-white">From Entity <span className="text-red-500">*</span></label>
              <div className="flex">
                ...
                <select value={fromEntity} onChange={(e) => setFromEntity(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  <option value="1653/EXPORT VARIETY">1653 / EXPORT VARIETY</option>
                  <option value="1736/SOUTH CHITHIRAI-MADURAI">1736 / SOUTH CHITHIRAI-MADURAI</option>
                  <option value="1185/DWH - COIMBATORE">1185 / DWH - COIMBATORE</option>
                  <option value="1881/PWH - SALEM">1881 / PWH - SALEM</option>
                  <option value="1781/PWH - MADURAI">1781 / PWH - MADURAI</option>
                  <option value="2181/PWH-KANCHIPURAM">2181 / PWH-KANCHIPURAM</option>
                </select>
              </div>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Stock Inward From <span className="text-red-500">*</span></label>
              <div className="flex">
                <IconBox><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg></IconBox>
                <select value={stockInwardFrom} onChange={(e) => setStockInwardFrom(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  <option value="1692/E Shopping">1692 / E Shopping</option>
                </select>
              </div>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Stock Movement Number / Date</label>
              <div className="flex">
                <IconBox><span className="text-sm font-bold">#</span></IconBox>
                <input type="text" value={stockMovementNumberDate} onChange={(e) => setStockMovementNumberDate(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 flex items-center justify-end gap-3">
            <button onClick={() => { setFromEntity(""); setStockInwardFrom(""); setStockMovementNumberDate("/"); setProducts([]); }} className="flex items-center gap-1.5 rounded bg-[#6c757d] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tfoot>
                  ...
                  <tr className="bg-white dark:bg-gray-dark">
                    <td colSpan={6} className="border border-stroke px-2 py-2 text-right font-semibold text-dark dark:border-dark-3 dark:text-white">Material Value(Without Tax)(&#8377;):</td>
                  ...
                  <tr className="bg-white dark:bg-gray-dark">
                    <td colSpan={6} className="border border-stroke px-2 py-2 text-right font-semibold text-dark dark:border-dark-3 dark:text-white">CGST(&#8377;)</td>
                  ...
                  <tr className="bg-white dark:bg-gray-dark">
                    <td colSpan={6} className="border border-stroke px-2 py-2 text-right font-semibold text-dark dark:border-dark-3 dark:text-white">SGST(&#8377;)</td>
                  ...
                  <tr className="bg-white dark:bg-gray-dark">
                    <td colSpan={6} className="border border-stroke px-2 py-2 text-right font-semibold text-dark dark:border-dark-3 dark:text-white">Net Total(&#8377;)</td>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-stroke pt-4 dark:border-dark-3">
            <button onClick={() => router.push("/operational/warehouse-management/ecommerce-warehouse/stock-acknowledgement/list")} className="flex items-center gap-1.5 rounded bg-[#6c757d] px-5 py-2.5 text-sm font-medium text-white hover:opacity-90">
