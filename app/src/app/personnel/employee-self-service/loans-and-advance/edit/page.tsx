...
export default function EditLoanAdvancePage() {
  const router = useRouter();

  // Pre-filled values
  const [loanType, setLoanType] = useState("LOAN");
  const [loanTypeDetail, setLoanTypeDetail] = useState("BTF-2");
  const [forwardTo, setForwardTo] = useState("Section Officer");
  const [forwardFor, setForwardFor] = useState("Approved");
  const [docs, setDocs] = useState<DocRow[]>([{ id: 1, name: "loan_application.pdf" }]);
  ...
  return (
    <div className="mx-auto">
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Loans &amp; Advance Request</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Edit Loans &amp; Advance Request</li>
          </ol>
        </nav>
      </div>
