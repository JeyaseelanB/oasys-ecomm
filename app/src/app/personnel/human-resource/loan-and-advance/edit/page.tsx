...
const INITIAL_DATA: Record<number, { employeeId: string; finType: string; forwardTo: string; forwardFor: string; documents: DocumentRow[] }> = {
  1: { employeeId: "422", finType: "MARRIAGE LOAN 2", forwardTo: "Branch Manager", forwardFor: "Approval", documents: [] },
  2: { employeeId: "759", finType: "CSD1", forwardTo: "HR Manager", forwardFor: "Approval", documents: [] },
  3: { employeeId: "687", finType: "CSD1", forwardTo: "HR Manager", forwardFor: "Review", documents: [] },
};
...
function EditContent() {
  ...
  const [finType, setFinType]         = useState(initial.finType);
  const [docFileName, setDocFileName] = useState("");
  const [documents, setDocuments]     = useState<DocumentRow[]>(initial.documents);
  const [forwardTo, setForwardTo]     = useState(initial.forwardTo);
  const [forwardFor, setForwardFor]   = useState(initial.forwardFor);
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Loans &amp; Advance</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Edit Loans &amp; Advance</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
          ...
          <div className="pb-5">
            ...
            <div className="mb-4 flex items-center gap-6">
              <label className="flex cursor-pointer items-center gap-2 text-sm text-dark dark:text-white">
                <input type="radio" name="editSelectBy" checked={selectBy === "employeeId"} onChange={() => setSelectBy("employeeId")} className="size-4 accent-primary" />
                By Employee Id
              </label>
              <label className="flex cursor-pointer items-center gap-2 text-sm text-dark dark:text-white">
                <input type="radio" name="editSelectBy" checked={selectBy === "entity"} onChange={() => setSelectBy("entity")} className="size-4 accent-primary" />
                By Entity
              </label>
            </div>
            {selectBy === "employeeId" ? (
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div>
                  ...
                  <div className="flex">
                    ...
                    <input type="text" value={employeeId} onChange={(e) => setEmployeeId(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                  </div>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div>
                  ...
                  <div className="flex">
                    ...
                    <input type="text" value={entityCode} onChange={(e) => setEntityCode(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="py-5">
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center gap-2">
                <input type="text" value={docFileName} readOnly placeholder="Choose file..." className="w-full max-w-xs rounded border border-stroke bg-transparent px-3 py-2 text-sm outline-none dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                <label className="flex cursor-pointer items-center gap-1.5 rounded bg-[#007bff] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
                  ...
                  Upload
                  <input type="file" accept=".pdf,.doc,.docx,.xlsx,.png,.jpg,.gif" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) setDocFileName(file.name); }} />
                </label>
              </div>
          ...
          <div className="pt-5">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <div className="flex">
                  <IconBox><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="5,12 12,5 19,12"/><polyline points="5,19 12,12 19,19"/></svg></IconBox>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-stroke px-5 py-4 dark:border-dark-3">
          ...
          <button onClick={() => router.push("/personnel/human-resource/loan-and-advance/list")} className="flex items-center gap-1.5 rounded bg-[#28a745] px-5 py-2.5 text-sm font-medium text-white hover:opacity-90">
...
export default function EditLoanAndAdvancePage() {
  return (
