...
interface DataItem { id: number; referenceNumber: string; recruitmentForPost: string; recruitmentYear: string; department: string; totalVacancies: string; previousRosterPoint: string; status: string; }
const SAMPLE_DATA: DataItem[] = [];
type SortKey = keyof DataItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function RosterReservationListPage() {
  const [filters, setFilters] = useState({ referenceNumber: "", recruitmentForPost: "", recruitmentYear: "", department: "", totalVacancies: "", previousRosterPoint: "", status: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) => row.referenceNumber.toLowerCase().includes(filters.referenceNumber.toLowerCase()) && (filters.recruitmentForPost === "" || row.recruitmentForPost.toLowerCase().includes(filters.recruitmentForPost.toLowerCase())) && row.recruitmentYear.toLowerCase().includes(filters.recruitmentYear.toLowerCase()) && (filters.department === "" || row.department === filters.department) && row.totalVacancies.toLowerCase().includes(filters.totalVacancies.toLowerCase()) && row.previousRosterPoint.toLowerCase().includes(filters.previousRosterPoint.toLowerCase()) && (filters.status === "" || row.status === filters.status));
  ...
  const handleClear = () => { setFilters({ referenceNumber: "", recruitmentForPost: "", recruitmentYear: "", department: "", totalVacancies: "", previousRosterPoint: "", status: "" }); setSelectedId(null); setCurrentPage(1); };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Roster Reservation List</h2>
        <nav><ol className="flex items-center gap-1.5 text-sm"><li><Link href="/" className="font-medium text-dark hover:text-primary dark:text-gray-400">Home</Link></li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Personnel</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Human Resource</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Recruitment Process</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Permanent Recruitment</li><li className="text-gray-400">/</li><li className="font-medium text-primary">Roster Reservation List</li></ol></nav>
      </div>
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white"><span className="text-primary">{filtered.length}</span> - Roster Reservation(s)</p>
          <div className="flex flex-wrap items-center gap-2">
            <button disabled={selectedId === null} className="flex items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14,2 14,8 20,8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/></svg>Create Job Advertisement</button>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("totalVacancies")}>Total Vacancies <SortIcon col="totalVacancies" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("previousRosterPoint")}>Previous Roster Point <SortIcon col="previousRosterPoint" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.totalVacancies} onChange={(e) => { setFilters((f) => ({ ...f, totalVacancies: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.previousRosterPoint} onChange={(e) => { setFilters((f) => ({ ...f, previousRosterPoint: e.target.value })); setCurrentPage(1); }} /></td>
