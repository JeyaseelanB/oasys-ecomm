...
const HORO_OPTIONS   = ["HEAD OFFICE", "VELLORE", "MADURAI", "COIMBATORE", "TRICHY", "SALEM", "CUDDALORE", "CHENNAI"];
const ENTITY_TYPES   = ["Head Office", "Regional Office", "D & P Office"];
...
const ENTITY_MAP: Record<string, string[]> = {
  ...
  "VELLORE":     ["21/VELLORE"],
  "MADURAI":     ["D&P Office Madurai"],
  "COIMBATORE":  ["COIMBATORE REGION"],
  "TRICHY":      ["TRICHY REGION"],
  "SALEM":       ["SALEM REGION"],
  "CUDDALORE":   ["D&P Office Cuddalore"],
  "CHENNAI":     ["CHENNAI REGION"],
};

const EMPLOYEE_MAP: Record<string, { name: string; pfNumber: string; designation: string; dateOfJoining: string; dateOfRetirement: string }[]> = {
  "HEAD OFFICE": [
    { name: "NAGARAJAN C",  pfNumber: "3265", designation: "DEPUTY GENERAL MANAGER CREDIT SALES", dateOfJoining: "08-Jul-1993", dateOfRetirement: "30-Jun-2034" },
    { name: "ARULRAJAN K",  pfNumber: "3234", designation: "MANAGER GRADE – I",                   dateOfJoining: "12-Mar-1995", dateOfRetirement: "31-Mar-2035" },
  ],
  "21/VELLORE": [
    { name: "SUJATHA V",    pfNumber: "3560", designation: "REGIONAL MANAGER INCHARGE",            dateOfJoining: "03-May-2013", dateOfRetirement: "31-Jul-2040" },
    { name: "MADHAVI V",    pfNumber: "1503", designation: "SENIOR OFFICER",                        dateOfJoining: "10-Jun-2000", dateOfRetirement: "30-Jun-2030" },
  ],
  "D&P Office Madurai": [
    { name: "JEYASATHYA S", pfNumber: "3553", designation: "JUNIOR ASSISTANT",                     dateOfJoining: "01-Apr-2015", dateOfRetirement: "30-Apr-2045" },
    { name: "GUNASEKARAN N",pfNumber: "990",  designation: "SENIOR ASSISTANT",                     dateOfJoining: "15-Jan-2005", dateOfRetirement: "31-Jan-2035" },
  ],
};

export default function CreateEDRPage() {
  const router = useRouter();
  const [horo, setHoro]               = useState("");
  const [entityType, setEntityType]   = useState("");
  const [entity, setEntity]           = useState("");
  const [empSelection, setEmpSelection] = useState("");
  const [natureOfDeath, setNatureOfDeath] = useState("");
  ...
  const entityOptions   = horo   ? (ENTITY_MAP[horo]   ?? []) : [];
  const employeeOptions = entity ? (EMPLOYEE_MAP[entity] ?? []) : [];
  const selectedEmp     = employeeOptions.find(e => `${e.name} / ${e.pfNumber}` === empSelection);

  const handleHoroChange = (v: string) => { setHoro(v); setEntity(""); setEmpSelection(""); };
  const handleEntityChange = (v: string) => { setEntity(v); setEmpSelection(""); };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Create Employee Death Registration</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Create Employee Death Registration</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="p-5 space-y-5">
          {/* Row 1: HO/RO, Entity Type, Entity, Employee Name */}
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <div className="flex">
                ...
                <select value={horo} onChange={e => handleHoroChange(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-[#2d8f7b] dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  {HORO_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              </div>
            </div>
            <div>
              ...
              <div className="flex">
                <IconBox><BuildingIcon /></IconBox>
                <select value={entityType} onChange={e => setEntityType(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-[#2d8f7b] dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
            ...
            <div>
              ...
              <div className="flex">
                <IconBox><BuildingIcon /></IconBox>
                <select value={entity} onChange={e => handleEntityChange(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-[#2d8f7b] dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
            ...
            <div>
              ...
              <div className="flex">
                ...
                <select value={empSelection} onChange={e => setEmpSelection(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-[#2d8f7b] dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="">Select</option>
                  {employeeOptions.map(e => <option key={e.pfNumber} value={`${e.name} / ${e.pfNumber}`}>{e.name} / {e.pfNumber}</option>)}
                </select>
              </div>
            </div>
          </div>

          {/* Row 2: PF Number, Designation, Date of Joining, Date of Retirement (read-only) */}
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-dark dark:text-white">Provident Fund Number</label>
              <div className="flex">
                <IconBox><HashIcon /></IconBox>
                <input readOnly value={selectedEmp?.pfNumber ?? ""} className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm text-[#2d8f7b] outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-white" />
              </div>
            </div>
            <div>
              ...
              <div className="flex">
                <IconBox><PersonIcon /></IconBox>
                <input readOnly value={selectedEmp?.designation ?? ""} className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm text-[#2d8f7b] outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-white" />
              </div>
            </div>
            <div>
              ...
              <div className="flex">
                ...
                <input readOnly value={selectedEmp?.dateOfJoining ?? ""} className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm text-[#2d8f7b] outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-white" />
              </div>
            </div>
            <div>
              ...
              <div className="flex">
                <IconBox><CalendarIcon /></IconBox>
                <input readOnly value={selectedEmp?.dateOfRetirement ?? ""} className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm text-[#2d8f7b] outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-white" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <div className="flex items-center rounded border border-stroke bg-transparent focus-within:border-[#2d8f7b] dark:border-dark-3 dark:bg-gray-dark">
                ...
                <button className="flex size-10 shrink-0 items-center justify-center rounded-r bg-[#17a2b8] text-white">
                  <CalendarIcon />
                </button>
              </div>
            </div>
            <div className="lg:col-span-2">
              ...
              <div className="flex items-center gap-2">
                <input readOnly value={fileName} placeholder="" className="flex-1 rounded border border-stroke bg-transparent px-3 py-2 text-sm outline-none dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                <label className="flex cursor-pointer items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
                  ...
                  Upload
                  <input type="file" className="hidden" accept=".png,.jpg,.JPG,.jpeg,.gif,.pdf,.doc,.docx,.xls,.xlsx" onChange={e => setFileName(e.target.files?.[0]?.name ?? "")} />
                </label>
              </div>
          ...
          <div className="flex items-center justify-between border-t border-stroke pt-4 dark:border-dark-3">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => router.push("/personnel/human-resource/retirement/employee-death-registration/list")}
                disabled={!horo || !entity || !empSelection || !natureOfDeath || !dateOfDeath}
                className="flex items-center gap-1.5 rounded bg-[#28a745] px-5 py-2.5 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <div className="w-full max-w-2xl rounded-[10px] bg-white shadow-lg dark:bg-gray-dark">
            ...
            <div className="p-5">
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <div className="w-48 rounded border border-stroke p-3 text-xs text-dark dark:border-dark-3 dark:text-white">
