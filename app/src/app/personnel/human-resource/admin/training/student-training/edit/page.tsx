...
export default function EditStudentTrainingPage() {
  const router = useRouter();
  const [showNoteModal, setShowNoteModal] = useState(false);

  // Pre-filled form fields
  const [institutionType, setInstitutionType] = useState("university");
  const [institutionName, setInstitutionName] = useState("Anna University");
  const [startDate, setStartDate] = useState("2024-01-15");
  const [endDate, setEndDate] = useState("2024-01-19");
  const [noOfDays, setNoOfDays] = useState("5");
  const [startTime, setStartTime] = useState("09:00 AM");
  const [endTime, setEndTime] = useState("05:00 PM");
  const [purpose, setPurpose] = useState("Industrial training for final year students to understand textile manufacturing processes.");
  const [topic, setTopic] = useState("Textile Manufacturing & Quality Control");
  // Institution details
  const [contactPerson, setContactPerson] = useState("Dr. Rajesh Kumar");
  const [contactNumber, setContactNumber] = useState("+91 98765 43210");
  const [email, setEmail] = useState("rajesh@annauniv.edu");
  const [address, setAddress] = useState("Anna University, Chennai - 600025, Tamil Nadu");
  const [noOfStudents, setNoOfStudents] = useState("20");
  const [department, setDepartment] = useState("Textile Technology");
  // Note modal
  ...
  return (
    <div className="mx-auto">
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Student Training Request</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Edit Student Training Request</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="p-5">
          ...
          <div className="mb-6 border-b border-stroke pb-6 dark:border-dark-3">
            ...
            <div className="mb-2">
              <p className="text-sm font-medium text-[#17a2b8]">training_request_form.pdf</p>
            </div>
            <input
              type="file"
              multiple
