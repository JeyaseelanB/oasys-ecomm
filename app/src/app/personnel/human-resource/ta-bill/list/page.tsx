...
type TABillStatus = "SUBMITTED";

interface TABillItem {
  id: number;
  taBillType: string;
  tourProgramPlanCode: string;
  claimName: string;
...
const SAMPLE_DATA: TABillItem[] = [
  { id: 1, taBillType: "TOUR_PROGRAM_CLAIM", tourProgramPlanCode: "Tentative", claimName: "", employeeCodeName: "252/SANKARANARAYANAN C", totalClaimAmount: 3580.0, status: "SUBMITTED" },
  { id: 2, taBillType: "OTHER_CLAIM", tourProgramPlanCode: "", claimName: "TA BILL", employeeCodeName: "518/PANDIYAMMAL R", totalClaimAmount: 482.0, status: "SUBMITTED" },
  { id: 3, taBillType: "OTHER_CLAIM", tourProgramPlanCode: "", claimName: "TA BILL", employeeCodeName: "518/PANDIYAMMAL R", totalClaimAmount: 598.0, status: "SUBMITTED" },
  { id: 4, taBillType: "OTHER_CLAIM", tourProgramPlanCode: "", claimName: "AS", employeeCodeName: "198/RAJESH KUMAR N", totalClaimAmount: 1200.0, status: "SUBMITTED" },
  { id: 5, taBillType: "", tourProgramPlanCode: "", claimName: "", employeeCodeName: "198/RAJESH KUMAR N", totalClaimAmount: 4112.0, status: "SUBMITTED" },
];

type SortKey = keyof TABillItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function TABillListPage() {
  ...
  const filtered = SAMPLE_DATA.filter((row) =>
    (filters.taBillType === "" || row.taBillType === filters.taBillType) &&
    row.tourProgramPlanCode.toLowerCase().includes(filters.tourProgramPlanCode.toLowerCase()) &&
    row.claimName.toLowerCase().includes(filters.claimName.toLowerCase()) &&
    row.employeeCodeName.toLowerCase().includes(filters.employeeCodeName.toLowerCase()) &&
    (filters.totalClaimAmount === "" || row.totalClaimAmount.toString().includes(filters.totalClaimAmount)) &&
  ...
  const handleClear = () => {
    setFilters({ taBillType: "", tourProgramPlanCode: "", claimName: "", employeeCodeName: "", totalClaimAmount: "", status: "" });
    setSelectedId(null);
    setCurrentPage(1);
  };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Travelling Allowance Bill List</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Travelling Allowance Bill List</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white">
            <span className="text-primary">{filtered.length}</span> - Travelling Allowance Bill(s)
          </p>
          <div className="flex flex-wrap items-center gap-2">
            <Link href="/personnel/human-resource/ta-bill/create" className="flex items-center gap-1.5 rounded bg-[#28a745] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
            ...
            <button
              onClick={() => { if (selectedId) router.push(`/personnel/human-resource/ta-bill/edit?id=${selectedId}`); }}
            ...
            <button
              onClick={() => { if (selectedId) router.push(`/personnel/human-resource/ta-bill/view?id=${selectedId}`); }}
              disabled={selectedId === null}
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("taBillType")}>TA Bill Type <SortIcon col="taBillType" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("tourProgramPlanCode")}>Tour Program Plan Code / Name <SortIcon col="tourProgramPlanCode" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("claimName")}>Claim Name <SortIcon col="claimName" /></th>
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("totalClaimAmount")}>Total Claim Amount (₹) <SortIcon col="totalClaimAmount" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
                  <select className="w-full rounded border border-stroke bg-transparent px-1 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" value={filters.taBillType} onChange={(e) => { setFilters((f) => ({ ...f, taBillType: e.target.value })); setCurrentPage(1); }}>
                    <option value="">Select</option>
                    <option value="TOUR_PROGRAM_CLAIM">TOUR_PROGRAM_CLAIM</option>
                    <option value="OTHER_CLAIM">OTHER_CLAIM</option>
                  </select>
                </td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.tourProgramPlanCode} onChange={(e) => { setFilters((f) => ({ ...f, tourProgramPlanCode: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.claimName} onChange={(e) => { setFilters((f) => ({ ...f, claimName: e.target.value })); setCurrentPage(1); }} /></td>
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.totalClaimAmount} onChange={(e) => { setFilters((f) => ({ ...f, totalClaimAmount: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3">
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="border-r border-stroke px-2 py-3 text-dark dark:border-dark-3 dark:text-white">{row.taBillType}</td>
                    <td className="border-r border-stroke px-2 py-3 text-dark dark:border-dark-3 dark:text-white">{row.tourProgramPlanCode}</td>
                    <td className="border-r border-stroke px-2 py-3 text-dark dark:border-dark-3 dark:text-white">{row.claimName}</td>
                    <td className="border-r border-stroke px-2 py-3 text-dark dark:border-dark-3 dark:text-white">{row.employeeCodeName}</td>
                    <td className="border-r border-stroke px-2 py-3 text-right text-dark dark:border-dark-3 dark:text-white">{row.totalClaimAmount.toFixed(2)}</td>
