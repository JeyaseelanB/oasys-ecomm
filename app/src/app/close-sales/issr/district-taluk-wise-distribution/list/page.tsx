...
interface DistributionItem {
  id: number;
  productionPlanCodeName: string;
  nameOfTheTaluk: string;
  createdDate: string;
}

const SAMPLE_DATA: DistributionItem[] = [
  { id: 1, productionPlanCodeName: "3356 / OAP DIWALI23", nameOfTheTaluk: "KARUR", createdDate: "28-03-2024" },
  { id: 2, productionPlanCodeName: "3412 / FDS PONGAL24", nameOfTheTaluk: "SALEM", createdDate: "15-01-2024" },
  { id: 3, productionPlanCodeName: "3289 / NMP ONAM23", nameOfTheTaluk: "ERODE", createdDate: "12-09-2023" },
  { id: 4, productionPlanCodeName: "3445 / OAP REPUBLIC24", nameOfTheTaluk: "COIMBATORE", createdDate: "20-01-2024" },
  { id: 5, productionPlanCodeName: "3356 / OAP DIWALI23", nameOfTheTaluk: "MADURAI", createdDate: "28-03-2024" },
];

type SortKey = keyof DistributionItem;
type SortDir = "asc" | "desc";
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

export default function DistrictTalukWiseListPage() {
  const router = useRouter();
  const [filters, setFilters] = useState({ productionPlanCodeName: "", nameOfTheTaluk: "", createdDate: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) =>
    row.productionPlanCodeName.toLowerCase().includes(filters.productionPlanCodeName.toLowerCase()) &&
    row.nameOfTheTaluk.toLowerCase().includes(filters.nameOfTheTaluk.toLowerCase()) &&
  ...
  const handleClear = () => { setFilters({ productionPlanCodeName: "", nameOfTheTaluk: "", createdDate: "" }); setSelectedId(null); setCurrentPage(1); };
  ...
  const basePath = "/close-sales/issr/district-taluk-wise-distribution";

  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">District / Taluk Wise List - Distribution</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">District / Taluk Wise List - Distribution</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white"><span className="text-primary">{filtered.length}</span> - District / Taluk Wise - Distribution(s)</p>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center align-middle font-semibold hover:bg-[#267a68]" onClick={() => handleSort("productionPlanCodeName")}>Production Plan Code / Name <SortIcon col="productionPlanCodeName" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center align-middle font-semibold hover:bg-[#267a68]" onClick={() => handleSort("nameOfTheTaluk")}>Name of The Taluk <SortIcon col="nameOfTheTaluk" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.productionPlanCodeName} onChange={(e) => { setFilters((f) => ({ ...f, productionPlanCodeName: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.nameOfTheTaluk} onChange={(e) => { setFilters((f) => ({ ...f, nameOfTheTaluk: e.target.value })); setCurrentPage(1); }} /></td>
              ...
              {paginated.length === 0 ? (<tr><td colSpan={5} className="py-8 text-center text-gray-400">No records found</td></tr>) : (
                paginated.map((row, idx) => (
                  <tr key={row.id} className={`border-b border-stroke dark:border-dark-3 ${idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"} hover:bg-blue-50 dark:hover:bg-[#1e2d42]`}>
                    ...
                    <td className="border-r border-stroke px-2 py-3 align-middle text-dark dark:border-dark-3 dark:text-white">{row.productionPlanCodeName}</td>
                    <td className="border-r border-stroke px-2 py-3 align-middle text-dark dark:border-dark-3 dark:text-white">{row.nameOfTheTaluk}</td>
