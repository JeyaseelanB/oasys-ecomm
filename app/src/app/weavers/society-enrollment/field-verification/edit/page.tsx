...
// Pre-filled data for the selected record (id=1, societyName="muru")
const PREFILL = {
  societyName: "muru",
  dateOfVisit: "21-Feb-2025",
  isRecommended: "yes" as "yes" | "no" | "",
  committeeMembers: [
    { id: 1, employeePf: "3200 / SAMINATHAN S", designation: "DEPUTY GENERAL MANAGER", entityCode: "10 / HEAD OFFICE" },
    { id: 2, employeePf: "1853 / RAJENDRAN V K", designation: "MANAGER GRADE - III", entityCode: "11 / COIMBATORE" },
  ] as CommitteeMember[],
  loomMembers: [
    { id: 1, memberName: "Rajan K", memberAddress: "12, Main Street, SALEM, TAMIL NADU - 636001", industryAddress: "45, Industrial Area, OMALUR, SALEM - 636455", numberOfLooms: "4", rationCard: "TN123456", aadhaar: "XXXX XXXX 1234" },
    { id: 2, memberName: "Priya M", memberAddress: "8, Cross Road, ERODE, TAMIL NADU - 638001", industryAddress: "Plot 7, SIDCO Estate, ERODE - 638002", numberOfLooms: "2", rationCard: "TN654321", aadhaar: "XXXX XXXX 5678" },
  ] as LoomMember[],
};
...
export default function FieldVerificationEditPage() {
  const router = useRouter();

  // Society — pre-filled
  const [societyName, setSocietyName] = useState(PREFILL.societyName);
  const [societyInfo, setSocietyInfo] = useState<typeof SOCIETY_INFO[string] | null>(SOCIETY_INFO[PREFILL.societyName]);

  // Committee Members — pre-filled
  const [dateOfVisit, setDateOfVisit] = useState(PREFILL.dateOfVisit);
  const [entityType, setEntityType] = useState("");
  const [entityCode, setEntityCode] = useState("");
  const [employeePf, setEmployeePf] = useState("");
  const [designation, setDesignation] = useState("");
  const [committeeMembers, setCommitteeMembers] = useState<CommitteeMember[]>(PREFILL.committeeMembers);
  const [committeeErrors, setCommitteeErrors] = useState<Record<string, string>>({});

  // Loom Verification — pre-filled
  ...
  const [loomMembers, setLoomMembers] = useState<LoomMember[]>(PREFILL.loomMembers);
  const [loomErrors, setLoomErrors] = useState<Record<string, string>>({});

  // Bottom — pre-filled
  const [isRecommended, setIsRecommended] = useState<"yes" | "no" | "">(PREFILL.isRecommended);
  ...
  return (
    <div className="mx-auto space-y-5">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Field Verification</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Edit Field Verification</li>
          </ol>
        </nav>
      </div>
