...
const SOCIETY_OPTIONS = [
  { value: "", label: "Select" },
  { value: "sri-shanmuga", label: "SRI SHANMUGA POWERLOOM WCS LTD" },
  { value: "erode-arignar", label: "ERODE ARIGNAR ANNA HL WCS" },
...
const SOCIETY_DETAILS: Record<string, SocietyData> = {
  "sri-shanmuga": {
    societyNameTamil: "SRI SHANMUGA POWERLOOM WCS LTD",
    societyType: "POWERLOOM",
    societyRegistrationNumber: "EH292",
    societyRegistrationDate: "09-Apr-2021",
    societyAddress: "sri ramanathan nagar, chithode, ERODE, TAMIL NADU, - 638102",
    societyProductionStartDate: "21-Jun-2021",
    societyResolutionDate: "16-Feb-2024",
    societyResolutionNumber: "1",
    adhtOfficeCode: "09 / ERODE",
    recommendedBy: "ASSISTANT DIRECTOR OF HANDLOOMS ERODE",
    recommendedDate: "21-Feb-2024",
    societyPresidentName: "elakkiyan",
    presidentAddress: "sri ramanathan nagar, chithode, ERODE, TAMIL NADU, - 638102",
    totalMembers: "40",
    demandDraftNumber: "424493",
    demandDraftDate: "17-Feb-2024",
    demandDraftAmount: "₹ 550.00",
    loomType: "POWERLOOM",
    numberOfLooms: "210",
    dpOfficeCode: "D&P Office Erode",
  },
  "abc-society": {
...
const LOOM_VERIFICATION = [
  { memberName: "GOKULAKRISHNAN", memberAddress: "sri ramanathan naga.......", industryAddress: "SRI RAMANATHAN NAGA.......", numberOfLooms: "6.0", rationCard: "", aadhaar: "123456781234" },
];
const VISIT_DATE = "23-Jan-2025";
...
export default function DepartmentApprovalAddPage() {
  ...
  const handleSubmit = () => {
    ...
    if (!forwardFor) errs.forwardFor = "Forward for is required";
    setErrors(errs);
    if (Object.keys(errs).length > 0) return;
    router.push("/weavers/society-enrollment/department-approval/list");
  };
  ...
  return (
    <div className="mx-auto space-y-5">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Create Head Office Approval</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Create Head Office Approval</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
        ...
        <div className="overflow-hidden">
          ...
          <div className="p-5">
            ...
            {/* Auto-populated society details */}
            {societyDetails && (
          ...
          <>
              ...
              <div className="flex cursor-pointer items-center justify-between bg-[#17a2b8] px-5 py-3" onClick={() => setDocsOpen((o) => !o)}>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="rounded-full bg-[#28a745] px-2 py-0.5 font-semibold text-white">Attached <span className="ml-1 rounded-full bg-white px-1.5 text-[#28a745]">12</span></span>
                    <span className="rounded-full bg-[#dc3545] px-2 py-0.5 font-semibold text-white">Remaining <span className="ml-1 rounded-full bg-white px-1.5 text-[#dc3545]">0</span></span>
                  </div>
                    ...
                    <tbody>
                      {SUPPORTING_DOCS.map((doc, idx) => (
                        <tr key={idx} className={idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-gray-50 dark:bg-[#1a2232]"}>
                          ...
                          <td className="border-b border-r border-stroke px-3 py-2 text-xs text-gray-500 dark:border-dark-3">/fileserver/INFOTEX-FILES/OPERATION/SOCIETY/ENROLLMENT/uploaded/DocScanner_23_Jan_2025_5_34_pm.pdf</td>
              ...
              {fieldOpen && (
                <div className="space-y-5 p-5">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      <label className="mb-1 block text-sm font-medium text-dark dark:text-white">Forward To <span className="text-red-500">*</span></label>
                      <div className="flex items-center gap-1">
                        <span className="flex size-8 shrink-0 items-center justify-center rounded border border-stroke bg-gray-50 dark:border-dark-3 dark:bg-dark-2">
                          <svg className="size-4 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="15,10 20,15 15,20"/><path d="M4 4v7a4 4 0 004 4h12"/></svg>
                        </span>
                        <input type="text" className="h-10 w-full rounded border border-stroke bg-transparent px-3 text-sm text-dark outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={forwardTo} onChange={(e) => setForwardTo(e.target.value)} />
                      </div>
                      {errors.forwardTo && <p className="mt-1 text-xs text-red-500">{errors.forwardTo}</p>}
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-dark dark:text-white">Forward For <span className="text-red-500">*</span></label>
                      <div className="flex items-center gap-1">
                        <span className="flex size-8 shrink-0 items-center justify-center rounded border border-stroke bg-gray-50 dark:border-dark-3 dark:bg-dark-2">
                          <svg className="size-4 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="15,10 20,15 15,20"/><path d="M4 4v7a4 4 0 004 4h12"/></svg>
                        </span>
                        <select className="h-10 w-full rounded border border-stroke bg-transparent px-3 text-sm text-dark outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" value={forwardFor} onChange={(e) => setForwardFor(e.target.value)}>
            ...
            <div className="flex items-center justify-between border-t border-stroke px-5 py-4 dark:border-dark-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => router.push("/weavers/society-enrollment/department-approval/list")} className="flex items-center gap-1.5 rounded bg-[#4b5563] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
