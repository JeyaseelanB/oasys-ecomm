...
/* ─── Sample data ─────────────────────────────────────────────────────────── */
interface PlanRow { id: number; code: string; name: string; createdDate: string; status: string; }

const SAMPLE_DATA: PlanRow[] = [
  { id: 1,  code: "thiyagu",             name: "thiyagu",             createdDate: "16-Apr-2025", status: "FINAL_APPROVED" },
  { id: 2,  code: "nighsn",              name: "nighsn",              createdDate: "16-Apr-2025", status: "SUBMITTED"      },
  { id: 3,  code: "yetsrdayyy",          name: "yetsrdayyy",          createdDate: "27-Mar-2025", status: "SUBMITTED"      },
  { id: 4,  code: "deepavali plans",     name: "deepavali plans",     createdDate: "27-Mar-2025", status: "SUBMITTED"      },
  { id: 5,  code: "oasys",              name: "oasys",              createdDate: "16-Apr-2025", status: "INITIATED"      },
  { id: 6,  code: "muruga",              name: "muruga",              createdDate: "25-Mar-2025", status: "SUBMITTED"      },
  { id: 7,  code: "Nishanth",            name: "Nishanth",            createdDate: "16-Apr-2025", status: "INITIATED"      },
  { id: 8,  code: "yuyuy",               name: "yuyuy",               createdDate: "05-Apr-2024", status: "SUBMITTED"      },
  { id: 9,  code: "2023_2024_ANNUAL_PLAN", name: "2023_2024_ANNUAL_PLAN", createdDate: "05-Apr-2023", status: "FINAL_APPROVED" },
  { id: 10, code: "23-24 yearly plan",   name: "23-24 yearly plan",   createdDate: "17-Mar-2023", status: "FINAL_APPROVED" },
  { id: 11, code: "PLAN_2024_Q1",        name: "PLAN_2024_Q1",        createdDate: "01-Jan-2024", status: "FINAL_APPROVED" },
  { id: 12, code: "PLAN_2024_Q2",        name: "PLAN_2024_Q2",        createdDate: "01-Apr-2024", status: "SUBMITTED"      },
  { id: 13, code: "ANNUAL_PLAN_2025",    name: "ANNUAL_PLAN_2025",    createdDate: "10-Jan-2025", status: "INITIATED"      },
  { id: 14, code: "QUARTERLY_2025_Q1",   name: "QUARTERLY_2025_Q1",   createdDate: "05-Jan-2025", status: "DRAFT"          },
  { id: 15, code: "MONTHLY_MAR_2025",    name: "MONTHLY_MAR_2025",    createdDate: "01-Mar-2025", status: "SUBMITTED"      },
];
...
/* ══════════════════════════════════════════════════════════════════════════ */
export default function ProductionPlanROListPage() {
  ...
  const [sortKey,     setSortKey]     = useState<keyof PlanRow>("id");
  const [sortAsc,     setSortAsc]     = useState(true);

  const toggleSort = (k: keyof PlanRow) => { if (sortKey === k) setSortAsc(!sortAsc); else { setSortKey(k); setSortAsc(true); } };
  ...
  const handleClear = () => { setSelectedId(null); setFilterCode(""); setFilterName(""); setFilterDate(""); setFilterStatus(""); setPage(1); };

  const SortIcon = ({ k }: { k: keyof PlanRow }) => (
  ...
  return (
    <div className="mx-auto">
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Production Plan RO List</h2>
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Production Plan RO List</li>
          </ol>
        </nav>
      </div>
        ...
        <div className="p-4">
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <span className="text-sm font-medium text-dark dark:text-white">
              {filtered.length} - Production Plan RO(s)
            </span>
            <div className="flex flex-wrap items-center gap-2">
              <button
                onClick={() => { if (selectedId) router.push(`${basePath}/list-showroom`); }}
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-[#2d8f7b] text-white">
                  ...
                  <th className="border border-[#3aa88f] px-3 py-2 text-center font-semibold cursor-pointer" onClick={() => toggleSort("code")}>
                    plan Code <SortIcon k="code" />
                  </th>
                  <th className="border border-[#3aa88f] px-3 py-2 text-center font-semibold cursor-pointer" onClick={() => toggleSort("name")}>
                    plan Name <SortIcon k="name" />
                  </th>
              ...
              <tbody>
                {paged.length === 0 ? (
                  <tr><td colSpan={6} className="border border-stroke px-3 py-4 text-center text-sm text-gray-400 dark:border-dark-3">No records found</td></tr>
                ) : (
                  paged.map((row, idx) => (
                    <tr key={row.id} className={idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"}>
                      ...
                      <td className="border border-stroke px-3 py-2 text-center text-dark dark:border-dark-3 dark:text-white">{row.code}</td>
                      <td className="border border-stroke px-3 py-2 text-center text-dark dark:border-dark-3 dark:text-white">{row.name}</td>
                      ...
                      <td className="border border-stroke px-3 py-2 text-center dark:border-dark-3">
                        <input type="radio" name="plan-select" checked={selectedId === row.id} onChange={() => setSelectedId(row.id)} className="accent-[#2d8f7b]" />
                      </td>
                    </tr>
                  ))
                )}
