...
/* ═══════════════════════════════════════════════════════════════
   PAGE
═══════════════════════════════════════════════════════════════ */
export default function CreatePrintingWarehouseStockOutwardPage() {
    ...
    <div className="mx-auto">
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">
          Create Stock Outward - Printing Warehouse
        </h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Create Stock Outward - Printing Warehouse</li>
          </ol>
        </nav>
      </div>

      {/* ── Section 1: Header (collapsible) ── */}
      <div className="mb-6 overflow-hidden rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
        <div
          className="flex cursor-pointer items-center justify-between bg-[#17a2b8] px-5 py-3"
          onClick={() => setHeaderOpen((o) => !o)}
        >
          <span className="font-semibold text-white">Create Stock Outward - Printing Warehouse</span>
      ...
      <div className="mb-6 rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="p-5">
          ...
          <div className="mb-5 flex flex-wrap items-end gap-4">
            ...
            <button className="flex h-[42px] items-center gap-1.5 rounded bg-[#17a2b8] px-5 text-sm font-medium text-white hover:opacity-90">
              <svg className="size-4" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="mb-6 rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="p-5">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
            <IconTextInput icon={<StackIcon />} label="Total Number of Bundles" value={totalBundles} onChange={setTotalBundles} />
            <IconTextInput icon={<StackIcon />} label="Bundle Number" value={bundleNum} onChange={setBundleNum} />
            <IconTextInput icon={<WeightIcon />} label="Total Bundle Weight" value={totalBundleWeight} onChange={setTotalBundleWeight} />
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="p-5">
          ...
          <div className="mb-5 grid grid-cols-1 gap-5 md:grid-cols-4">
            <IconSelect icon={<TruckIcon />} label="Transport Service Type" options={["BY ROAD", "BY RAIL", "BY AIR", "BY SEA"]} value={transportServiceType} onChange={setTransportServiceType} />
            <IconSelect icon={<TruckIcon />} label="Transport Service Name" options={["Door Delivery", "Self Pickup"]} value={transportServiceName} onChange={setTransportServiceName} />
            <IconSelect icon={<StackIcon />} label="Waybill Available" options={["Yes", "No"]} value={waybillAvailable} onChange={setWaybillAvailable} />
            <IconTextInput icon={<FileIcon />} label="Waybill Number" value={waybillNumber} onChange={setWaybillNumber} />
          </div>
          <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
            <IconSelect icon={<StackIcon />} label="Transport Charge Available" options={["Yes", "No"]} value={transportChargeAvailable} onChange={setTransportChargeAvailable} />
            <IconSelect icon={<FileIcon />} label="Transport Charge Type" options={["ToPay", "Paid", "FOC"]} value={transportChargeType} onChange={setTransportChargeType} />
            <IconTextInput icon={<RupeeIcon />} label="Transport Charge Amount" value={transportChargeAmount} onChange={setTransportChargeAmount} />
          </div>
        </div>
      </div>
