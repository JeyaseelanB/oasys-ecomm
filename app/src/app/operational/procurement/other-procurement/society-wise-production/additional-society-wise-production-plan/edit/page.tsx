...
const MOCK_SOCIETIES: SocietyRow[] = [
...
export default function EditAdditionalSocietyWisePlanPage() {
  ...
  const [planType]     = useState("Additional Production Plan");
  const [planCode, setPlanCode]         = useState("APP-SEP-25-73 / Prodcution plan");
  const [planFrom, setPlanFrom]         = useState("01-Sep-2025");
  const [planTo, setPlanTo]             = useState("09-Sep-2025");
  const [createdDate, setCreatedDate]   = useState("08-Sep-2025");
  const [createdBy, setCreatedBy]       = useState("hosuper");
  const [approvedDate, setApprovedDate] = useState("08-Sep-2025");
  const [approvedBy, setApprovedBy]     = useState("hosuper");

  const [productCategory, setProductCategory] = useState("A / Pure Silk Variety");
  const [productGroup, setProductGroup]       = useState("30 / SILK ITEM");
  const [productVariety, setProductVariety]   = useState("SKBS / SAREES KPM SILK WITH BLOUSE");

  const [societies, setSocieties] = useState<SocietyRow[]>(MOCK_SOCIETIES);
  const [forwardTo, setForwardTo]   = useState("");
  const [forwardFor, setForwardFor] = useState("Final Approve");

  const [showCreateNote, setShowCreateNote] = useState(false);
  const [noteContent, setNoteContent]       = useState("Tewsting");

  const handleQtyChange = (id: number, val: string) => setSocieties(societies.map((s) => s.id === id ? { ...s, currentQty: val } : s));
  const totalQty = societies.reduce((s, r) => s + (parseFloat(r.currentQty) || 0), 0);
  const currentReqQty = totalQty;

  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Edit Additional Society Wise Production Plan</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Edit Additional Society Wise Production Plan</li>
          </ol>
        </nav>
      </div>
        ...
        <div className="p-5">
          ...
          <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-4">
            <div>
              ...
              <div className="flex">
                ...
                <input type="text" readOnly value={planType} className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm text-gray-600 outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-gray-400" />
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex">
                ...
                <select value={planCode} onChange={(e) => setPlanCode(e.target.value)} className="w-full rounded-r border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white">
                  <option value="APP-SEP-25-73 / Prodcution plan">APP-SEP-25-73 / Prodcution plan</option>
                  <option value="APP-AUG-25-69 / dilli">APP-AUG-25-69 / dilli</option>
                </select>
              </div>
            </div>
            <div>
              ...
              <div className="flex">
                <input type="text" value={planFrom} onChange={(e) => setPlanFrom(e.target.value)} className="w-full rounded-l border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                <CalBox />
              </div>
            </div>
            <div>
              ...
              <div className="flex">
                <input type="text" value={planTo} onChange={(e) => setPlanTo(e.target.value)} className="w-full rounded-l border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                <CalBox />
              </div>
            </div>
            <div>
              ...
              <div className="flex">
                <input type="text" value={createdDate} onChange={(e) => setCreatedDate(e.target.value)} className="w-full rounded-l border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                <CalBox />
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <div className="flex">
                <input type="text" value={approvedDate} onChange={(e) => setApprovedDate(e.target.value)} className="w-full rounded-l border border-stroke bg-transparent px-3 py-2 text-sm outline-none focus:border-primary dark:border-dark-3 dark:bg-gray-dark dark:text-white" />
                <CalBox />
              </div>
            </div>
            <div>
          ...
          <div className="mt-5">
            ...
            <div className="mb-3 grid grid-cols-1 gap-4 md:grid-cols-4">
              <div>
                ...
                <div className="flex">
                  <IconBox><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><line x1="4" y1="9" x2="20" y2="9"/><line x1="4" y1="15" x2="20" y2="15"/><line x1="10" y1="3" x2="8" y2="21"/><line x1="16" y1="3" x2="14" y2="21"/></svg></IconBox>
                  <input readOnly value="2107 / D&P OFFICE KANCHIPURAM" className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm text-gray-500 outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-gray-400" />
                </div>
              </div>
              <div>
            ...
            <div className="mb-3 grid grid-cols-1 gap-4 md:grid-cols-4">
              <div>
                ...
                <div className="flex">
                  <IconBox><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M12 2a10 10 0 100 20A10 10 0 0012 2z"/></svg></IconBox>
                  <input readOnly value="NOS" className="w-full rounded-r border border-stroke bg-gray-50 px-3 py-2 text-sm text-gray-500 outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-gray-400" />
                </div>
              </div>
            </div>
            <div className="mb-4 flex justify-end gap-3">
              <button className="flex items-center gap-1.5 rounded bg-[#6c757d] px-5 py-2 text-sm font-medium text-white hover:opacity-90">
          ...
          <div className="mt-5">
            ...
            <div className="flex items-center justify-between border-t border-stroke pt-4 dark:border-dark-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button disabled className="flex cursor-not-allowed items-center gap-1.5 rounded bg-[#17a2b8] px-5 py-2.5 text-sm font-medium text-white opacity-50">
            ...
            <div className="p-5">
              <div className="rounded-t border border-stroke dark:border-dark-3">
                <div className="flex flex-wrap items-center gap-0.5 border-b border-stroke bg-[#f9fafb] px-2 py-1.5 dark:border-dark-3 dark:bg-[#1a2232]">
                  <select className="mr-1 rounded border border-stroke bg-transparent px-1.5 py-1 text-xs outline-none dark:border-dark-3 dark:bg-gray-dark dark:text-white"><option>Sans Serif</option></select>
                  <select className="mr-1 rounded border border-stroke bg-transparent px-1.5 py-1 text-xs outline-none dark:border-dark-3 dark:bg-gray-dark dark:text-white"><option>Normal</option></select>
                  ...
                  <span className="mx-0.5 h-5 w-px bg-stroke dark:bg-dark-3"></span>
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200"><span className="text-sm font-bold">A</span></button>
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200"><span className="text-[10px]">x<sub>2</sub></span></button>
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200"><span className="text-[10px]">x<sup>2</sup></span></button>
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200"><span className="text-xs font-bold">H<sub>1</sub></span></button>
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200"><span className="text-xs font-bold">H<sub>2</sub></span></button>
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200"><svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg></button>
                  <button className="flex size-7 items-center justify-center rounded text-gray-600 hover:bg-gray-200"><svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="16,18 22,12 16,6"/><polyline points="8,6 2,12 8,18"/></svg></button>
                </div>
              </div>
              <div className="mb-4 min-h-[180px] rounded-b border border-t-0 border-stroke p-3 dark:border-dark-3">
                <textarea value={noteContent} onChange={(e) => setNoteContent(e.target.value)} rows={7} className="w-full resize-none bg-transparent text-sm outline-none dark:text-white" />
              </div>
              <div className="relative mb-6 flex items-center justify-between">
                <button className="flex size-8 items-center justify-center rounded-full border border-stroke text-gray-400 hover:bg-gray-100 dark:border-dark-3"><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="15,18 9,12 15,6"/></svg></button>
                ...
                <button className="flex size-8 items-center justify-center rounded-full border border-stroke text-gray-400 hover:bg-gray-100 dark:border-dark-3"><svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="9,18 15,12 9,6"/></svg></button>
              </div>
