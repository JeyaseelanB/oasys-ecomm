...
type OrderStatus = "APPROVED" | "SUBMITTED" | "REJECTED" | "INITIATED";
...
const SAMPLE_DATA: ProcurementOrderItem[] = [
  { id: 1, planCode: "RPPY1818-8/Plan for Co-Optex", orderCode: "SPH18T03", dnpOfficeCode: "1806/D&P Office Salem",       categoryCode: "AJ/Half Fine Silk", status: "APPROVED"  },
  { id: 2, planCode: "RPPY1818-8/Plan for Co-Optex", orderCode: "EPH18T03", dnpOfficeCode: "1301/D&P OFFICE ERODE",       categoryCode: "AJ/Half Fine Silk", status: "APPROVED"  },
  { id: 3, planCode: "RPPY1818-8/Plan for Co-Optex", orderCode: "GPH18T03", dnpOfficeCode: "1106/NMP CFDS CENTRE ERODE",  categoryCode: "AJ/Half Fine Silk", status: "REJECTED"  },
  { id: 4, planCode: "RPPY1919-1/Retail_Plan_2019",  orderCode: "SPH19T01", dnpOfficeCode: "1806/D&P Office Salem",       categoryCode: "C/Cotton Variety",  status: "APPROVED"  },
  { id: 5, planCode: "RPPY1919-1/Retail_Plan_2019",  orderCode: "CPH19T01", dnpOfficeCode: "2107/D&P OFFICE KANCHIPURAM", categoryCode: "C/Cotton Variety",  status: "SUBMITTED" },
];

type SortKey = keyof ProcurementOrderItem;
...
export default function ProcurementOrderListPage() {
  ...
  const [filters, setFilters] = useState({ planCode: "", orderCode: "", dnpOfficeCode: "", categoryCode: "", status: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) =>
    ...
    row.orderCode.toLowerCase().includes(filters.orderCode.toLowerCase()) &&
    row.dnpOfficeCode.toLowerCase().includes(filters.dnpOfficeCode.toLowerCase()) &&
    row.categoryCode.toLowerCase().includes(filters.categoryCode.toLowerCase()) &&
  ...
  const handleClear = () => {
    setFilters({ planCode: "", orderCode: "", dnpOfficeCode: "", categoryCode: "", status: "" });
    setSelectedId(null);
    setCurrentPage(1);
  };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Retail Sales – Procurement Order List</h2>
        <nav>
          <ol className="flex flex-wrap items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li><li className="font-medium text-primary">Retail Sales – Procurement Order List</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white">
            <span className="text-primary">{filtered.length}</span> - Procurement Order(s)
          </p>
          <div className="flex flex-wrap items-center gap-2">
            <Link href="/operational/procurement/retail-procurement/procurement-order/create"
            ...
            <button onClick={() => { if (selectedId) router.push("/operational/procurement/retail-procurement/procurement-order/create"); }} disabled={selectedId === null}
            ...
            <button onClick={() => { if (selectedId) router.push("/operational/procurement/retail-procurement/procurement-order/view"); }} disabled={selectedId === null}
              className="flex items-center gap-1.5 rounded bg-[#17a2b8] px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("planCode")}>Production Plan Code / Name <SortIcon col="planCode" /></th>
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("orderCode")}>Procurement Order Code <SortIcon col="orderCode" /></th>
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-3 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("categoryCode")}>Product Category Code / Name <SortIcon col="categoryCode" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.orderCode} onChange={(e) => { setFilters((f) => ({ ...f, orderCode: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.dnpOfficeCode} onChange={(e) => { setFilters((f) => ({ ...f, dnpOfficeCode: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.categoryCode} onChange={(e) => { setFilters((f) => ({ ...f, categoryCode: e.target.value })); setCurrentPage(1); }} /></td>
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((row, idx) => (
                  <tr key={row.id} onClick={() => setSelectedId(row.id)}
                    ...
                    <td className="border-r border-stroke px-3 py-2.5 text-dark dark:border-dark-3 dark:text-white">{row.orderCode}</td>
                    <td className="border-r border-stroke px-3 py-2.5 text-dark dark:border-dark-3 dark:text-white">{row.dnpOfficeCode}</td>
                    <td className="border-r border-stroke px-3 py-2.5 text-dark dark:border-dark-3 dark:text-white">{row.categoryCode}</td>
