...
const ListIco = () => (
  <svg className="w-3.5 h-3.5 text-gray-500" viewBox="0 0 20 20" fill="none" stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 6h12M4 10h12M4 14h12" />
  </svg>
);
...
const Sel = ({
  label, required, icon, options, value, onChange,
}: {
...
const Inp = ({
  label, required, icon, value, onChange, readOnly, placeholder, type,
}: {
  label: string; required?: boolean; icon: React.ReactNode;
  ...
  <F label={label} required={required} icon={icon}>
    <input
      type={type ?? "text"} readOnly={readOnly} placeholder={placeholder}
...
const DateF = ({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) => (
  <div className="flex flex-col gap-1">
    ...
    <div className="flex border border-gray-300 rounded overflow-hidden h-8">
      ...
      <div className="px-2 flex items-center justify-center border-l border-gray-300 cursor-pointer" style={{ backgroundColor: "#17a2b8" }}>
        <CalIco />
      </div>
    </div>
  </div>
...
type ItemRow = {
  id: number; itemName: string; itemDesc: string; uom: string; qty: string;
  unitRate: string; totalRate: string; discountValue: string; taxValue: string; netAmount: string;
};

export default function EditPurchaseOrderPage() {
  const router = useRouter();

  const [supplierType, setSupplierType] = useState("SOCIETY");
  const [supplierCode, setSupplierCode] = useState("");
  const [orderBasedOn, setOrderBasedOn] = useState("Without Quotation");
  ...
  const addTaxRow = () => {
    if (!taxType) return;
    setTaxRows((r) => [...r, { id: Date.now(), taxType, tax, taxValue: taxValueInput }]);
    setTaxType(""); setTax(""); setTaxValueInput("");
  };
  const addItemRow = () => {
    if (!itemName && !prodVariety) return;
    setItemRows((r) => [...r, { id: Date.now(), itemName, itemDesc, uom, qty: quantity, unitRate, totalRate: itemTotal, discountValue: discValue, taxValue: taxValueInput, netAmount }]);
  };
    ...
    <div className="p-4">
      <nav className="mb-1 text-sm text-gray-500">
        <ol className="flex flex-wrap items-center gap-1">
          ...
          <li>/</li>
          <li className="text-gray-700">Edit Purchase Order</li>
        </ol>
      </nav>
      <h1 className="text-base font-semibold text-gray-800 mb-3">Edit Purchase Order</h1>
      ...
      <div className="bg-white rounded shadow-sm border border-gray-200 mb-4">
        ...
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Sel label="Supplier Type" required icon={<ListIco />} options={["SOCIETY", "GENSUP", "LOCSUP"]} value={supplierType} onChange={setSupplierType} />
            <Inp label="Supplier Code / Name" required icon={<HashIco />} value={supplierCode} onChange={setSupplierCode} />
            <Sel label="Purchase Order Based on" required icon={<ListIco />} options={["With Quotation", "Without Quotation"]} value={orderBasedOn} onChange={setOrderBasedOn} />
          </div>
          <div className="flex justify-end gap-2 mt-3">
            <button className="flex items-center gap-1.5 px-4 py-1.5 text-white text-sm font-semibold rounded" style={{ backgroundColor: "#6c757d" }}
              onClick={() => { setSupplierType(""); setSupplierCode(""); setOrderBasedOn(""); }}>
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M4 13.5V17h3.5l9.026-9.026-3.5-3.5L4 13.5z" /></svg>
              Clear
            </button>
      ...
      <div className="bg-white rounded shadow-sm border border-gray-200 mb-4">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex gap-4">
            <div className="flex-1 space-y-3">
              ...
              <div className="flex gap-3 items-end">
                ...
                <button className="flex items-center gap-1.5 px-4 py-1.5 text-white text-sm font-semibold rounded mb-0.5" style={{ backgroundColor: "#28a745" }} onClick={addTaxRow}>
              ...
              <div className="flex gap-2">
                <button className="flex items-center gap-1.5 px-4 py-1.5 text-white text-sm font-semibold rounded" style={{ backgroundColor: "#6c757d" }}
                  onClick={() => { setGstApplicable(""); setTaxType(""); setTax(""); setTaxValueInput(""); setNetAmount(""); setQuantity(""); setUnitRate(""); setItemTotal(""); setDiscValue(""); }}>
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M4 13.5V17h3.5l9.026-9.026-3.5-3.5L4 13.5z" /></svg>
                  Clear
                </button>
                <button className="flex items-center gap-1.5 px-4 py-1.5 text-white text-sm font-semibold rounded" style={{ backgroundColor: "#28a745" }} onClick={addItemRow}>
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                  Add
                </button>
              </div>
            ...
            <div className="w-80">
              <table className="w-full text-xs border border-gray-200 rounded overflow-hidden">
                ...
                <tbody className="bg-white">
                  {taxRows.length === 0 ? (
                    <tr><td colSpan={5} className="px-3 py-3 text-gray-500 text-xs">No records found.</td></tr>
                  ) : (
                    taxRows.map((r, i) => (
                      <tr key={r.id} className="border-b border-gray-100">
                        ...
                        <td className="px-2 py-1.5 text-center"><button className="text-red-500 hover:text-red-700 text-xs" onClick={() => delTax(r.id)}>✕</button></td>
                      </tr>
                    ))
                  )}
          ...
          <div className="overflow-x-auto rounded border border-gray-200">
            <table className="min-w-full text-xs">
              ...
              <tbody className="bg-white">
                {itemRows.length === 0 ? (
                  <tr><td colSpan={11} className="px-3 py-3 text-gray-500 text-xs">No records found.</td></tr>
                ) : (
                  itemRows.map((r, i) => (
                    <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-2 py-1.5"><button className="text-red-500 hover:text-red-700 text-xs" onClick={() => delItem(r.id)}>✕</button></td>
                    </tr>
                  ))
                )}
          ...
          <div className="flex gap-4 mt-2">
            ...
            <div className="flex-1 flex flex-col gap-1">
              ...
              <textarea rows={5} className="flex-1 border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:border-teal-500 resize-none" value={terms} onChange={(e) => setTerms(e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-gray-200">
          <button className="flex items-center gap-1.5 px-4 py-1.5 text-white text-sm font-semibold rounded" style={{ backgroundColor: "#6c757d" }} onClick={() => router.push("/operational/quotation-order-invoice/purchase/purchase-order/list")}>
          ...
          <div className="bg-white rounded shadow-xl w-full max-w-2xl mx-4">
            ...
            <div className="p-4 space-y-3">
              <div className="flex flex-wrap gap-1 border border-gray-200 rounded p-1.5 bg-gray-50">
                <select className="text-xs border border-gray-300 rounded px-1 py-0.5"><option>Font</option><option>Arial</option><option>Times New Roman</option></select>
                <select className="text-xs border border-gray-300 rounded px-1 py-0.5"><option>Size</option>{[10,12,14,16,18].map(s => <option key={s}>{s}</option>)}</select>
              ...
              <textarea rows={5} className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:border-teal-500 resize-none" placeholder="Enter note..." value={noteText} onChange={(e) => setNoteText(e.target.value)} />
              <div className="flex items-start gap-4">
                <div className="border border-gray-300 rounded p-3 flex-1">
                  ...
                  <p className="text-xs text-gray-700">Name : ALOK</p>
                ...
                <div className="flex flex-col items-center gap-2 pt-2">
                  <div className="flex gap-1"><span className="w-2 h-2 rounded-full bg-teal-600 inline-block"></span></div>
