...
const INITIAL_ITEMS: RequirementItem[] = [
...
export default function EditRequirementPage() {
  ...
  const [items, setItems] = useState<RequirementItem[]>(INITIAL_ITEMS);
  ...
  const handleQtyChange = (id: number, value: string) => {
    setItems((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, quantityRequired: value === "" ? 0 : parseFloat(value) || 0 }
          : item
      )
    );
  };
  ...
  const handleUpdate = () => {
    ...
    router.push("/operational/printing-stationary/requirement/list");
  };
    ...
    <div className="mx-auto">
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
          <ol className="flex items-center gap-1.5 text-sm">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-primary">Edit Requirement</li>
          </ol>
        </nav>
      </div>
      ...
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <div className="flex items-center justify-between rounded-t-[10px] bg-[#2dc4b2] px-5 py-3">
          <h3 className="text-sm font-semibold text-white">Edit Requirement</h3>
        ...
        <div className="grid grid-cols-1 gap-x-6 border-b border-stroke px-6 py-5 dark:border-dark-3 sm:grid-cols-2">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-sm font-medium text-[#2dc4b2]">{INITIAL_DATA.requirementCodeName}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Category Group Code / Name</span>
            <span className="text-sm font-medium text-[#2dc4b2]">{INITIAL_DATA.categoryGroupCodeName}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-x-6 border-b border-stroke px-6 py-5 dark:border-dark-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-sm font-medium text-[#2dc4b2]">{INITIAL_DATA.sectionCodeName}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Requirement From Date</span>
            <span className="text-sm font-medium text-[#2dc4b2]">{INITIAL_DATA.fromDate}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Requirement To Date</span>
            <span className="text-sm font-medium text-[#2dc4b2]">{INITIAL_DATA.toDate}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Due Date</span>
            <span className="text-sm font-medium text-[#2dc4b2]">{INITIAL_DATA.dueDate}</span>
          </div>
        </div>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-[#2d8f7b] text-white">
                  ...
                  <th className="w-24 border border-[#3aa88f] px-3 py-3 text-center font-semibold">UOM</th>
