...
const SAMPLE_DATA: DataItem[] = [
  { id: 1, referenceNumber: "A885", hoRo: "Head Office", entityType: "Head Office", entity: "HEAD OFFICE", assetName: "PANK", createdDate: "29-Nov-2024" },
  { id: 2, referenceNumber: "A884", hoRo: "Head Office", entityType: "Head Office", entity: "HEAD OFFICE", assetName: "New Sagwan", createdDate: "28-Nov-2024" },
  { id: 3, referenceNumber: "A883", hoRo: "SALEM", entityType: "Showroom", entity: "KARUR", assetName: "Fancing", createdDate: "04-Nov-2024" },
  { id: 4, referenceNumber: "A882", hoRo: "Regional OfficeCUDDALORE", entityType: "Regional Office", entity: "CUDDALORE", assetName: "Dining Table", createdDate: "04-Nov-2024" },
  { id: 5, referenceNumber: "A881", hoRo: "Head Office", entityType: "Head Office", entity: "HEAD OFFICE", assetName: "Machine", createdDate: "04-Nov-2024" },
  { id: 6, referenceNumber: "A880", hoRo: "Head Office", entityType: "Head Office", entity: "HEAD OFFICE", assetName: "ihwehywyr", createdDate: "24-Jul-2024" },
  { id: 7, referenceNumber: "A879", hoRo: "VELLORE", entityType: "Showroom", entity: "DEEPAM- VELLORE", assetName: "GLOBE PRESSING LOCK", createdDate: "19-Feb-2024" },
  { id: 8, referenceNumber: "A878", hoRo: "VELLORE", entityType: "Showroom", entity: "DEEPAM- VELLORE", assetName: "PEDESTAL FANS", createdDate: "19-Feb-2024" },
  { id: 9, referenceNumber: "A877", hoRo: "VELLORE", entityType: "Showroom", entity: "DEEPAM- VELLORE", assetName: "CROMPTON WALL FAN", createdDate: "19-Feb-2024" },
  { id: 10, referenceNumber: "A876", hoRo: "VELLORE", entityType: "Showroom", entity: "DEEPAM- VELLORE", assetName: "Fans", createdDate: "19-Feb-2024" },
];
...
export default function AssetRegisterListPage() {
  const basePath = "/masters/asset-masters/asset-register";
  const [filters, setFilters] = useState({ referenceNumber: "", hoRo: "", entityType: "", entity: "", assetName: "", createdDate: "" });
  ...
  const filtered = SAMPLE_DATA.filter((row) =>
    ...
    row.assetName.toLowerCase().includes(filters.assetName.toLowerCase()) &&
  ...
  const handleClear = () => { setFilters({ referenceNumber: "", hoRo: "", entityType: "", entity: "", assetName: "", createdDate: "" }); setSelectedId(null); setCurrentPage(1); };
  ...
  return (
    <div className="mx-auto">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[22px] font-bold leading-tight text-dark dark:text-white">Asset Register List</h2>
        <nav><ol className="flex items-center gap-1.5 text-sm"><li><Link href="/" className="font-medium text-dark hover:text-primary dark:text-gray-400">Home</Link></li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Masters</li><li className="text-gray-400">/</li><li className="text-gray-500 dark:text-gray-400">Asset Masters</li><li className="text-gray-400">/</li><li className="font-medium text-primary">Asset Register List</li></ol></nav>
      </div>
      <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stroke px-5 py-4 dark:border-dark-3">
          <p className="text-sm font-semibold text-dark dark:text-white"><span className="text-primary">{filtered.length}</span> - Asset Register(s)</p>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#2d8f7b] text-white">
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("hoRo")}>H.O / R.O <SortIcon col="hoRo" /></th>
                ...
                <th className="cursor-pointer border border-[#3aa88f] px-2 py-3 text-center font-semibold hover:bg-[#267a68]" onClick={() => handleSort("assetName")}>Asset Name <SortIcon col="assetName" /></th>
              ...
              <tr className="bg-white dark:bg-gray-dark">
                ...
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.entityType} onChange={(e) => { setFilters((f) => ({ ...f, entityType: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.entity} onChange={(e) => { setFilters((f) => ({ ...f, entity: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" value={filters.assetName} onChange={(e) => { setFilters((f) => ({ ...f, assetName: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"><input type="text" className="w-full rounded border border-stroke bg-transparent px-2 py-1 text-xs outline-none focus:border-primary dark:border-dark-3 dark:text-white" placeholder="dd-MMM-yyyy" value={filters.createdDate} onChange={(e) => { setFilters((f) => ({ ...f, createdDate: e.target.value })); setCurrentPage(1); }} /></td>
                <td className="border border-stroke px-1 py-1.5 dark:border-dark-3"></td>
              </tr>
            </thead>
            <tbody>{paginated.length === 0 ? (<tr><td colSpan={8} className="py-8 text-center text-gray-400">No records found</td></tr>) : paginated.map((row, idx) => (<tr key={row.id} className={`border-b border-stroke dark:border-dark-3 ${selectedId === row.id ? "bg-[#2d8f7b] text-white" : idx % 2 === 0 ? "bg-white dark:bg-gray-dark" : "bg-[#f9fafb] dark:bg-[#1a2232]"} ${selectedId !== row.id ? "hover:bg-blue-50 dark:hover:bg-[#1e2d42]" : ""}`}>
              ...
              <td className={`border-r px-2 py-3 ${selectedId === row.id ? "border-[#3aa88f]" : "border-stroke dark:border-dark-3 text-dark dark:text-white"}`}>{row.hoRo}</td>
              <td className={`border-r px-2 py-3 ${selectedId === row.id ? "border-[#3aa88f]" : "border-stroke dark:border-dark-3 text-dark dark:text-white"}`}>{row.entityType}</td>
              ...
              <td className={`border-r px-2 py-3 ${selectedId === row.id ? "border-[#3aa88f]" : "border-stroke dark:border-dark-3 text-dark dark:text-white"}`}>{row.assetName}</td>
